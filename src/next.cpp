...
#include "next.hpp"
#include "common.hpp"
#include "src/part/ext2_common.hpp"
#include <algorithm>
#include <config.h>
#include <cstdint>
...
struct search_location_t
{
  uint64_t offset;
  uint64_t inc;
};
static inline auto CHS_to_offset(const unsigned int C, const int H, const int S,
                                 const disk_t &disk_car) -> uint64_t;
...
static void update_location()
{
  ...
  for (i = 0; i < search_location_nbr; i++)
  {
    ...
    if (cur->offset == src->offset && cur->inc >= src->inc &&
        cur->inc % src->inc == 0)
    {
      cur->inc = src->inc;
      return;
    ...
    {
      if (cur->offset == src->offset && src->inc >= cur->inc &&
          src->inc % cur->inc == 0)
        return;
      if (cur->inc == src->inc && cur->offset >= src->offset &&
          (cur->offset - src->offset) % cur->inc == 0)
      {
        cur->offset = src->offset;
        return;
      }
      if (cur->inc == src->inc && src->offset >= cur->offset &&
                          ...
                          const int fast_mode)
{
  ...
  if (disk_car.arch == &arch_i386)
  {
    if (fast_mode > 1)
    {
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 0, 1, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(0, 1, 0, disk_car);
      update_location();
    }
    else
    {
      ...
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 0, 1, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(1, 0, 0, disk_car);
      update_location();
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 1, 1, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(1, 0, 0, disk_car);
      update_location();
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 2, 1, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(1, 0, 0, disk_car);
    }
  ...
  if (fast_mode > 0)
  {
    ...
    if (disk_car.arch == &arch_i386)
    {
      // CHS H=0,1,2 S=7
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 0, 7, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(1, 0, 0, disk_car);
      update_location();
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 1, 7, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(1, 0, 0, disk_car);
      update_location();
      search_location_info[search_location_nbr].offset =
          CHS_to_offset(0, 2, 7, disk_car);
      search_location_info[search_location_nbr].inc =
          CHS_to_offset(1, 0, 0, disk_car);
      update_location();
    }
    ...
    /* test_nbr==5*/
    {
      ...
      for (s_log_block_size = 0; s_log_block_size <= 2; s_log_block_size++)
      {
        ...
        {
          search_location_info[search_location_nbr].offset =
              CHS_to_offset(0, 0, 1, disk_car) + hd_offset;
          search_location_info[search_location_nbr].inc =
              CHS_to_offset(1, 0, 0, disk_car);
          update_location();
          search_location_info[search_location_nbr].offset =
              CHS_to_offset(0, 1, 1, disk_car) + hd_offset;
          search_location_info[search_location_nbr].inc =
              CHS_to_offset(1, 0, 0, disk_car);
          update_location();
          search_location_info[search_location_nbr].offset =
              CHS_to_offset(0, 2, 1, disk_car) + hd_offset;
          search_location_info[search_location_nbr].inc =
              CHS_to_offset(1, 0, 0, disk_car);
          update_location();
        }
...
auto search_location_update(const uint64_t location) -> uint64_t
{
  ...
  for (i = 0; i < search_location_nbr; i++)
  {
    ...
    min = std::min(min, search_location_info[i].offset);
  }
  return min;
}
