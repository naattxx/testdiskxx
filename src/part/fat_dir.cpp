...
auto dir_fat_aux(const unsigned char *buffer, const unsigned int size,
                 const unsigned int param, dir_list_t &dir_list) -> int
{
...
GetNew:
  ...
  ParseLongDeleted:
    ...
    while (true)
    {
      if (reinterpret_cast<const void *>(de) >=
...
RecEnd:
  ...
  de++;
  if (reinterpret_cast<const void *>(de) <
          reinterpret_cast<const void *>(buffer + size - 1) &&
...
/*@
  ...
  @ assigns \nothing;
  @*/
static auto is_EOC(const unsigned int cluster, const upart_type_t upart_type)
    -> int
{
                    ...
                    dir_list_t &dir_list) -> int
{
  ...
  }
  {
    ...
    nbr_cluster   = 0;
    while (!is_EOC(cluster, partition.upart_type) && cluster >= 2 &&
           nbr_cluster < nbr_cluster_max && stop == 0)
    {
      const uint64_t start =
          partition.part_offset +
          (start_data + (cluster - 2) * fat_header->sectors_per_cluster) *
      ...
      if (std::cmp_not_equal(disk_car.pread(disk_car,
                                            buffer_dir + static_cast<uint64_t>(
                                                             cluster_size
                                                         ) * nbr_cluster,
                                            cluster_size, start),
                             cluster_size))
      ...
      if (stop == 0)
      {
        if (fat_meth == FAT_FOLLOW_CLUSTER)
        {
          const unsigned int next_cluster =
              get_next_cluster(disk_car, partition, partition.upart_type,
                               start_fat1, cluster);
