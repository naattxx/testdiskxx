...
/*@
  ...
  @*/
static void set_NTFS_info(disk_t &disk_car,
...
/*@
  ...
  @ decreases 0;
  @*/
static void ntfs_get_volume_name(disk_t &disk_car, partition_t &partition,
                                 const struct ntfs_boot_sector *ntfs_header);
                ...
                const int dump_ind) -> int
{
   ...
   * unsigned)partition.part_offset); */
  ...
  if (backup > 0)
  {
    if (partition.part_offset + disk_car.sector_size < part_size)
    {
      log_warning("NTFS part_offset={}, part_size={}, sector_size={}\n",
                  ...
                  (long long unsigned)part_size, disk_car.sector_size);
  ...
  if (partition.part_size > 0)
  {
    ...
    if (part_size * ntfs_sector_size(ntfs_header) > partition.part_size)
    {
      screen_buffer_add("Error: size boot_sector {} > partition {}\n",
                        ...
                        static_cast<long unsigned>(partition.part_size /
...
/*@
  ...
  @ assigns  \nothing;
  @*/
static auto ntfs_getattributeheaders(const ntfs_recordheader *record)
...
/*@
  ...
  @ assigns  \nothing;
  @*/
static auto ntfs_searchattribute(const ntfs_attribheader *attrib,
                                 uint32_t attrType, const char *end, int skip)
    -> const ntfs_attribheader *
{
  if (attrib == nullptr)
    return nullptr;
         ...
         le32(attrib->type) != 0xffffffff)
  {
    ...
    if (!skip)
    {
      ...
      skip = 0;
    attrib = reinterpret_cast<const ntfs_attribheader *>(
        reinterpret_cast<const char *>(attrib) + attr_len
    );
  }
  return nullptr;
}
                                 ...
                                 const struct ntfs_boot_sector *ntfs_header)
{
  ...
  if (ntfs_header->clusters_per_mft_record > 0)
    mft_record_size = ntfs_header->clusters_per_mft_record *
  ...
  }
  {
    ...
    if (attrib && attrib->header.bNonResident == 0) /* attribute is resident */
    {
      ...
      for (dest = partition.fsname.data();
        ...
        *dest++ = *name_it;
      *dest = '\0'; /* 27 january 2003: Correct a bug found by Andreas du
                       Plessis-Denz */
    }
  }
  delete[] (buffer);
