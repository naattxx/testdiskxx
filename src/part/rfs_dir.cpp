...
#endif
        if (my_data->disk_car->pwrite(my_data->disk_car, buff, blocklen, my_data->partition->part_offset + off) !=
            blocklen)
            return 0;
        return 1;
    }
...
static int file_equals(dal_t *dal1, dal_t *dal2)
{
    ...
    /* return !strcmp((char *)dal1->data, (char *)dal2->data); */
    {
        ...
        return (data1->disk_car == data2->disk_car) && (data1->partition == data2->partition) &&
                      ...
                      const unsigned long int cluster, file_info_t *dir_list)
{
    ...
    while (reiserfs_dir_read(dir, &entry))
    {
        ...
        if ((entity = reiserfs_object_create(ls->current_fs, name, 1)))
        {
            ...
            file_info_t *new_file = (file_info_t *)new unsigned char[sizeof(*new_file)];
            thislen =
                (MAX_NAME_LEN(DEFAULT_BLOCK_SIZE) < DIR_NAME_LEN ? MAX_NAME_LEN(DEFAULT_BLOCK_SIZE) : DIR_NAME_LEN);
                                          ...
                                          const int verbose)
{
    ...
    {
        struct rfs_dir_struct *ls = (struct rfs_dir_struct *)new unsigned char[sizeof(*ls)];
        ls->dir_list = NULL;
        ls->current_fs = fs;
        ls->dal = dal;
