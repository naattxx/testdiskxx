...
static int file_read(dal_t *dal, void *buff, blk_t block, blk_t count)
{
  ...
  my_data_t *my_data;
  /* log_trace("reiser file_read(dal=%p,buff=%p,block={},
...
#endif
  /* log_debug("blocklen={}\n",blocklen); */
  if (my_data->disk_car->pread(my_data->disk_car, buff, blocklen,
                               my_data->partition.part_offset + off) !=
      blocklen)
    return 0;
  return 1;
}
...
#endif
    if (my_data->disk_car->pwrite(my_data->disk_car, buff, blocklen,
                                  my_data->partition.part_offset + off) !=
        blocklen)
      return 0;
    return 1;
...
static int file_equals(dal_t *dal1, dal_t *dal2)
{
  ...
  /* return !strcmp((char *)dal1->data, (char *)dal2->data); */
  {
    ...
    return (data1->disk_car == data2->disk_car) &&
           (data1->partition == data2->partition) &&
...
#ifdef HAVE_STRUCT_DAL_OPS_DEV
dal_t *dal_open(struct dal_ops *myops, const void *dev, size_t blocksize,
                int flags, void *data)
#else
dal_t *dal_open(struct dal_ops *myops, unsigned blocksize, int flags,
                void *data)
#endif
{
  dal_t *dal;
  ...
  while (reiserfs_dir_read(dir, &entry))
  {
    ...
    if ((entity = reiserfs_object_create(ls->current_fs, name, 1)))
    {
      ...
      file_info_t *new_file =
          (file_info_t *)new unsigned char[sizeof(*new_file)];
      thislen = (MAX_NAME_LEN(DEFAULT_BLOCK_SIZE) < DIR_NAME_LEN
                     ? MAX_NAME_LEN(DEFAULT_BLOCK_SIZE)
                     : DIR_NAME_LEN);
                               ...
                               dir_data_t *dir_data, const file_info_t *file)
{
  ...
  {
    void *buf=new unsigned char[file_size+1];
    ...
    -> dir_partition_t
{
...
#endif
  {
    ...
    /* file_close call delete (my_data) */
    file_close(dal);
    return DIR_PART_EIO;
  }
