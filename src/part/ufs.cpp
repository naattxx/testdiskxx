...
static auto test_ufs(const disk_t &disk_car, const struct ufs_super_block *sb, const partition_t &partition,
                     const int verbose) -> int;

auto check_ufs(disk_t &disk_car, partition_t &partition, const int verbose) -> int
{
                     ...
                     const int verbose) -> int
{
    if (le32(sb->fs_magic) == UFS_MAGIC && le32(sb->fs_size) > 0 &&
        (le32(sb->fs_fsize) == 512 || le32(sb->fs_fsize) == 1024 || le32(sb->fs_fsize) == 2048 ||
    ...
    if (be32(sb->fs_magic) == UFS_MAGIC && be32(sb->fs_size) > 0 &&
        (be32(sb->fs_fsize) == 512 || be32(sb->fs_fsize) == 1024 || be32(sb->fs_fsize) == 2048 ||
...
auto recover_ufs(const disk_t &disk_car, const struct ufs_super_block *sb, partition_t &partition, const int verbose,
                 const int dump_ind) -> int
{
    ...
    {
    case UP_UFS_LE:
        partition.part_size = static_cast<uint64_t> le32(sb->fs_size) * le32(sb->fs_fsize);
        if (verbose > 1)
        {
    ...
    case UP_UFS2_LE:
        ...
        break;
    case UP_UFS:
        partition.part_size = static_cast<uint64_t>(be32(sb->fs_size)) * be32(sb->fs_fsize);
        if (verbose > 1)
        {
        ...
        break;
    case UP_UFS2:
        partition.part_size = static_cast<uint64_t>(be64(sb->fs_u11.fs_u2.fs_size)) * be32(sb->fs_fsize);
        if (verbose > 1)
        {
...
static void set_ufs_info(const struct ufs_super_block *sb, partition_t &partition)
{
    ...
    if (le32(sb->fs_magic) == UFS_MAGIC)
    {
        ...
        partition.set_name(reinterpret_cast<const char *>(sb->fs_u11.fs_u1.fs_fsmnt),
    ...
    if (le32(sb->fs_magic) == UFS2_MAGIC)
    {
        ...
        partition.set_name(reinterpret_cast<const char *>(sb->fs_u11.fs_u2.fs_fsmnt),
