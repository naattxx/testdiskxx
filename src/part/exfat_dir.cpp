...
#ifdef HAVE_ICONV
static int exfat_ucstoutf8(iconv_t cd, const unsigned char *ins, const unsigned int ins_len, char **outs,
                           const unsigned int outs_len)
{
    const char *inp;
    char *outp;
                     ...
                     const unsigned long int first_cluster, dir_list_t &dir_list)
{
    ...
    unsigned char *buffer_dir = new unsigned char[NBR_CLUSTER_MAX << cluster_shift];
    ...
    {
        if (exfat_read_cluster(disk, partition, exfat_header, buffer_dir + ((uint64_t)nbr_cluster << cluster_shift),
