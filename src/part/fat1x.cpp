...
#ifdef HAVE_NCURSES
static void dump_fat1x_ncurses(disk_t &disk_car, const partition_t &partition,
...
static void dump_fat1x(disk_t &disk_car, const partition_t &partition,
                       const unsigned char *buffer_bs, char **current_cmd)
{
                       ...
                       const unsigned int expert, char **current_cmd) -> int
{
  ...
  while (true)
  {
    ...
    screen_buffer_reset();
    {
      ...
      if (disk_car.pread(disk_car, buffer_bs, FAT1x_BOOT_SECTOR_SIZE,
                         partition.part_offset) != FAT1x_BOOT_SECTOR_SIZE)
      {
