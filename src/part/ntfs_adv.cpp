                                                ...
                                                const unsigned char *newboot, const int expert)
{
    ...
    while (1)
    {
        ...
        command = wmenuSelect(stdscr, INTER_NTFSBS_Y + 1, INTER_NTFSBS_Y, INTER_NTFSBS_X, menuSaveBoot, 8, options,
                                    ...
                                    const uint32_t index_block_size, const int expert, char **current_cmd)
{
    ...
    ntfs_header->clusters_per_mft_record =
        (mft_record_size >= cluster_size ? mft_record_size / cluster_size : -(testdisk_ffs(mft_record_size) - 1));
    ntfs_header->clusters_per_index_record =
        (index_block_size >= cluster_size ? index_block_size / cluster_size : -(testdisk_ffs(index_block_size) - 1));
                    ...
                    char **current_cmd)
{
    ...
    /* Find partition location using MFT information */
    {
        ...
        for (i = 0; i < nbr_mft; i++)
        {
            ...
            {
                if (info_mft[i].mft_lcn == info_mft[j].mft_lcn && info_mft[i].mftmirr_lcn == info_mft[j].mftmirr_lcn &&
