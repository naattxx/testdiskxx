...
static int ncurses_ntfs2_info(const struct ntfs_boot_sector *nh1,
...
static void ntfs_dump_ncurses(disk_t &disk_car, const partition_t &partition,
...
static void ntfs_dump(disk_t &disk_car, const partition_t &partition,
                      const unsigned char *orgboot,
...
static void ntfs_write_boot_sector(disk_t &disk, partition_t &partition,
                                   const unsigned char *newboot)
{
  ...
  if (disk.pwrite(disk, newboot, NTFS_SECTOR_SIZE, partition.part_offset) !=
      NTFS_SECTOR_SIZE)
  {
  ...
  if (disk.pwrite(disk, newboot, NTFS_SECTOR_SIZE,
                  partition.part_offset + partition.part_size -
                      disk.sector_size) != NTFS_SECTOR_SIZE)
  {
    ; // display_message("Write error: Can't write new NTFS backup boot
      // sector\n");
  }
  disk.sync(disk);
}

static void ntfs_list(disk_t &disk, const partition_t &partition,
                      const unsigned char *newboot, char **current_cmd,
                      const int expert)
{
...
static void menu_write_ntfs_boot_sector_cli(disk_t &disk_car,
                                            ...
                                            const int expert)
{
  const auto *org_ntfs_header =
      reinterpret_cast<const struct ntfs_boot_sector *>(orgboot);
  const auto *ntfs_header =
      reinterpret_cast<const struct ntfs_boot_sector *>(newboot);
  int no_confirm = 0;
  while (true)
  {
...
#ifdef HAVE_NCURSES
static void menu_write_ntfs_boot_sector_ncurses(disk_t &disk_car,
                                                partition_t &partition,
                                                const unsigned char *orgboot,
                                                const unsigned char *newboot,
                                                const int expert)
{
  const struct ntfs_boot_sector *org_ntfs_header =
      (const struct ntfs_boot_sector *)orgboot;
  const struct ntfs_boot_sector *ntfs_header =
      (const struct ntfs_boot_sector *)newboot;
  ...
  while (1)
  {
    ...
    command =
        wmenuSelect(stdscr, INTER_NTFSBS_Y + 1, INTER_NTFSBS_Y, INTER_NTFSBS_X,
...
static void create_ntfs_boot_sector(disk_t &disk_car, partition_t &partition,
                                    const unsigned int cluster_size,
                                    const uint64_t mft_lcn,
                                    const uint64_t mftmirr_lcn,
                                    const uint32_t mft_record_size,
                                    const uint32_t index_block_size,
                                    const int expert, char **current_cmd)
{
  ...
  if (disk_car.pread(disk_car, &orgboot, NTFS_SECTOR_SIZE,
                     partition.part_offset) != NTFS_SECTOR_SIZE)
  {
  ...
  */
  ...
  {
    menu_write_ntfs_boot_sector_cli(disk_car, partition, orgboot, newboot,
                                    current_cmd, expert);
    return;
  }
#ifdef HAVE_NCURSES
...
static auto read_mft_info(disk_t &disk_car, const partition_t &partition,
                          const uint64_t mft_sector, const int verbose,
                          unsigned int *sectors_per_cluster, uint64_t *mft_lcn,
                          uint64_t *mftmirr_lcn, unsigned int *mft_record_size)
    -> int
{
  ...
  if (disk_car.pread(disk_car, &buffer, sizeof(buffer),
                     partition.part_offset +
                         mft_sector * disk_car.sector_size) != sizeof(buffer))
  {
  ...
  *mft_record_size = le32(record->bytes_allocated);
  ...
  attr80 = reinterpret_cast<const ntfs_attribnonresident *>(
      ntfs_findattribute(record, 0x80, buffer + sizeof(buffer))
  );
  if (attr80 && attr80->header.bNonResident)
  {
    *mft_lcn = ntfs_get_first_rl_element(attr80, buffer + sizeof(buffer));
  }
  record = reinterpret_cast<const struct ntfs_mft_record *>(buffer +
                                                            (*mft_record_size));
                     ...
                     char **current_cmd) -> int
{
       ...
       sector++)
  {
    if (disk_car.pread(disk_car, &buffer, 0x400,
                       partition.part_offset + sector * static_cast<uint64_t>(
                                                        ...
                                                        )) == 0x400)
    {
          ...
          le16(record->flags) == 1) /* MFT_RECORD_IN_USE */
      {
        ...
        {
          const auto *file_name_attr =
              reinterpret_cast<const TD_FILE_NAME_ATTR *>(ntfs_getattributedata(
                  reinterpret_cast<const ntfs_attribresident *>(attr30),
                  buffer + 0x400
              ));
          if (file_name_attr != nullptr &&
              file_name_attr->file_name_length == 4 &&
              reinterpret_cast<const char *>(&file_name_attr->file_name[0]) +
                      8 <=
                  buffer + 0x400 &&
        ...
        if (res == 1)
        {
          ...
          tmp = read_mft_info(disk_car, partition, sector, verbose,
                              &sectors_per_cluster, &mft_lcn, &mftmirr_lcn,
                              &mft_record_size);
          if (tmp == 0)
          {
  ...
  for (sector = 1;
       (sector < partition.part_size / disk_car.sector_size) && (ind_stop == 0);
       sector++)
  {
#ifdef HAVE_NCURSES
    if ((sector & 0xffff) == 0)
  ...
  if (expert > 0)
  {
    ...
    sectors_per_cluster =
        ask_number(sectors_per_cluster, 0, 512, "Sectors per cluster ");
    wmove(stdscr, INTER_NTFS_Y, INTER_NTFS_X);
    ...
    wmove(stdscr, INTER_NTFS_Y, INTER_NTFS_X);
    mft_record_size =
        ask_number(mft_record_size, 42, 4096, " mft record size ");
  }
#endif
  /* TODO
   * read_mft_info(partition,sector,*sectors_per_cluster,*mft_lcn,*mftmirr_lcn,*mft_record_size);
   */
  if (sectors_per_cluster > 0 && mft_record_size >= 42 &&
      mft_record_size <= sizeof(buffer))
  {
    // 0x90 AT_INDEX_ROOT
    ...
    if (std::cmp_not_equal(
            disk_car.pread(disk_car, &buffer, mft_record_size,
                           partition.part_offset +
                               mft_lcn * sectors_per_cluster *
                                   disk_car.sector_size +
    ...
    attr90 =
        ntfs_findattribute(reinterpret_cast<const ntfs_recordheader *>(buffer),
                           0x90, buffer + mft_record_size);
    if (attr90 && attr90->bNonResident == 0)
    {
      const auto *index_root =
          reinterpret_cast<const TD_INDEX_ROOT *>(ntfs_getattributedata(
              reinterpret_cast<const ntfs_attribresident *>(attr90),
    ...
    create_ntfs_boot_sector(disk_car, partition,
                            sectors_per_cluster * disk_car.sector_size, mft_lcn,
                            mftmirr_lcn, mft_record_size, index_block_size,
                            expert, current_cmd);
