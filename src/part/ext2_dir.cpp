...
static auto my_read_blk(io_channel channel, unsigned long block, int count,
                        void *buf) -> errcode_t;
static auto my_write_blk(io_channel channel, unsigned long block, int count,
                         const void *buf) -> errcode_t;
static auto my_flush(io_channel channel) -> errcode_t;
static auto my_read_blk64(io_channel channel, unsigned long long block,
                          int count, void *buf) -> errcode_t;
 ...
 */
static auto alloc_io_channel(const disk_t &disk_car, my_data_t *my_data)
    -> io_channel
{
  io_channel ioch;
#ifdef DEBUG_EXT2
...
#ifdef DEBUG_EXT2
  log_info(
      "my_read_blk start size={}, offset={} name={}, block={}, count={}, "
      "buf={:p}",
...
static auto list_dir_proc2(ext2_ino_t dir, int entry,
                           struct ext2_dir_entry *dirent, int offset,
                           int blocksize, char *buf, void *privateinfo) -> int
{
  ...
  {
    log_error("ext2fs_read_inode(ino={}) failed with error {}.", (unsigned)ino,
              (long)retval);
    return 0;
  }
  if (inode.i_mode == 0)
  ...
  new_file.st_size =
      LINUX_S_ISDIR(inode.i_mode)
          ? inode.i_size
          : inode.i_size | (static_cast<uint64_t>(inode.i_size_high) << 32);
