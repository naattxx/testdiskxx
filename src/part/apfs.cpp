...
auto recover_APFS(const disk_t &disk, const nx_superblock_t *sb, partition_t &partition, const int verbose,
                  const int dump_ind) -> int
{
    ...
    guid_cpy(&partition.part_uuid, reinterpret_cast<const efi_guid_t *>(&sb->nx_uuid));
    if (verbose > 0)
    {
        log_info("\n");
    }
