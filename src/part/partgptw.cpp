...
static auto find_gpt_entry(const uint64_t lba_start,
                           const struct gpt_ent *gpt_entries_org) -> int
{
  int i;
...
static void partition_generate_gpt_entry(struct gpt_ent *gpt_entry,
                                         const partition_t &partition,
                                         const disk_t &disk_car,
                                         const struct gpt_ent *gpt_entries_org)
{
                    ...
                    const int ro, const int verbose) -> int
{
  ...
  for (const partition_t &element : list_part)
  {
    ...
    {
      partition_generate_gpt_entry(&gpt_entries[element.order - 1], element,
                                   disk_car, gpt_entries_org);
    }
  }
  gpt = reinterpret_cast<struct gpt_hdr *>(
      new unsigned char[disk_car.sector_size]
  );
  gpt_org = reinterpret_cast<struct gpt_hdr *>(
      new unsigned char[disk_car.sector_size]
  );
