...
static auto find_gpt_entry(const uint64_t lba_start, const struct gpt_ent *gpt_entries_org) -> int
{
  int i;
  if (gpt_entries_org == nullptr)
      return -1;
  for(i=0; i<128; i++)
  {
...
static void partition_generate_gpt_entry(struct gpt_ent* gpt_entry, const partition_t &partition, const disk_t &disk_car, const struct gpt_ent* gpt_entries_org)
{
  ...
  str2UCSle(reinterpret_cast<uint16_t *>(&gpt_entry->ent_name), partition.partname, sizeof(gpt_entry->ent_name) / 2);
  if(entry >= 0)
...
static auto write_part_gpt_i386(disk_t &disk_car, const list_part_t &list_part) -> int
{
  /* The Protective MBR has the same format as a legacy MBR. */
  list_part_t list_part_i386;
  uint64_t efi_psize=disk_car.disk_size;
  partition_t const *part_mac = nullptr;
  partition_t const *part_linux = nullptr;
  partition_t const *part_windows = nullptr;
  for(const partition_t &element : list_part)
  {
      if (part_mac == nullptr && element.part_type_i386 == P_HFS)
          part_mac = &element;
      else if (part_linux == nullptr && element.part_type_i386 == P_LINUX)
          part_linux = &element;
      else if (part_windows == nullptr && element.part_type_i386 == P_NTFS)
          part_windows = &element;
  }
  if (part_mac != nullptr && (part_linux != nullptr || part_windows != nullptr))
  { /* For bootcamp, the layout should be
    ...
    {
      int insert_error=0;
      partition_t new_partition = *part_mac;
      new_partition.arch=&arch_i386;
      new_partition.status=STATUS_PRIM;
      new_partition.order=2;
      insert_new_partition(list_part_i386, new_partition, 0, &insert_error);
      if(insert_error == 0 && efi_psize > new_partition.part_offset)
        efi_psize=new_partition.part_offset;
    }
    if (part_linux != nullptr)
    {
      int insert_error=0;
      partition_t new_partition = *part_linux;
      new_partition.arch=&arch_i386;
      new_partition.status=STATUS_PRIM;
    ...
    if (part_windows != nullptr)
    {
      int insert_error=0;
      partition_t new_partition = *part_windows;
      new_partition.arch=&arch_i386;
      new_partition.status=STATUS_PRIM;
...
auto write_part_gpt(disk_t &disk_car, const list_part_t &list_part, const int ro, const int verbose) -> int
{
  struct gpt_hdr *gpt;
  ...
  if(ro>0)
    return 0;
  gpt_entries_org = reinterpret_cast<struct gpt_ent *>(new unsigned char[gpt_entries_size]);
  ...
  for(const partition_t &element:list_part)
  {
    ...
    {
      partition_generate_gpt_entry(&gpt_entries[element.order-1],
          element, disk_car, gpt_entries_org);
    }
  }
  gpt = reinterpret_cast<struct gpt_hdr *>(new unsigned char[disk_car.sector_size]);
  gpt_org = reinterpret_cast<struct gpt_hdr *>(new unsigned char[disk_car.sector_size]);
  if (std::cmp_equal(disk_car.pread(disk_car, gpt_org, disk_car.sector_size, disk_car.sector_size),
  ...
  if (std::cmp_not_equal(
          disk_car.pwrite(disk_car, gpt_entries, gpt_entries_size, le64(gpt->hdr_lba_table) * disk_car.sector_size),
          gpt_entries_size))
  {
    delete[](gpt_org);
  ...
  if (std::cmp_not_equal(
          disk_car.pwrite(disk_car, gpt, disk_car.sector_size, le64(gpt->hdr_lba_self) * disk_car.sector_size),
          disk_car.sector_size))
  {
    delete[](gpt_org);
    delete[](gpt_entries_org);
