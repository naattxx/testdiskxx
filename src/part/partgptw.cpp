...
static int write_part_gpt_i386(disk_t &disk_car, const list_part_t &list_part)
{
  ...
  partition_t const *part_mac=NULL;
  partition_t const *part_linux=NULL;
  partition_t const *part_windows=NULL;
  for(const partition_t &element : list_part)
  {
    if(part_mac==NULL && element.part_type_i386==P_HFS)
      part_mac=&element;
    else if(part_linux==NULL && element.part_type_i386==P_LINUX)
      part_linux=&element;
    else if(part_windows==NULL && element.part_type_i386==P_NTFS)
...
int write_part_gpt(disk_t &disk_car, const list_part_t &list_part, const int ro, const int verbose)
{
  ...
  if(ro>0)
    return 0;
  gpt_entries_org=(struct gpt_ent*)new unsigned char[gpt_entries_size];
  ...
  gpt_org=(struct gpt_hdr*)new unsigned char[disk_car.sector_size];
