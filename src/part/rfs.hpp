...
struct [[gnu::gcc_struct, gnu::packed]] reiserfs_super_block
{
  ...
  uint32_t
      s_journal_trans_max; /* 0x18 max number of blocks in a transaction.  */
  uint32_t s_journal_block_count; /* 0x1C total size of the journal. can change
                                     over time  */
  uint32_t
      s_journal_max_batch; /* 0x20 max number of blocks to batch into a trans */
  uint32_t s_journal_max_commit_age; /* 0x24 in seconds, how old can an async
                                        commit be */
  uint32_t s_journal_max_trans_age;  /* 0x28 in seconds, how old can a
                                        transaction be */
  uint16_t s_blocksize;              /* 0x2C block size           */
  uint16_t s_oid_maxsize;            /* 0x2E max size of object id array, see
                                        get_objectid() commentary  */
  uint16_t s_oid_cursize;            /* 0x30 current size of object id array */
  uint16_t s_state;                  /* 0x32 valid or error       */
  char s_magic[10]; /* 0x34 reiserfs magic string indicates that file system is
                       reiserfs */
  uint16_t sb_fs_state; /* 0x3E it is set to used by fsck to mark which phase of
               rebuilding is done (used for fsck debugging) */
  uint32_t s_hash_function_code; /* 0x40 indicate, what hash fuction is being
                                    use to sort names in a directory*/
  uint16_t s_tree_height;        /* 0x44 height of disk tree */
  uint16_t s_bmap_nr;  /* 0x46 amount of bitmap blocks needed to address each
