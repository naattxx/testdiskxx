...
#define MD_SB_RESERVED_WORDS                                                  \
  (1024 - MD_SB_GENERIC_WORDS - MD_SB_PERSONALITY_WORDS - MD_SB_DISKS_WORDS - \
...
struct mdp_superblock_1
{
  ...
  uint64_t recovery_offset; /* sectors before this offset (from data_offset)
                               have been recovered */
  uint32_t
      dev_number; /* permanent identifier of this  device - not role in raid */
...
#define WriteMostly1 1         /* mask for writemostly flag in above */
  ...
  uint64_t resync_offset; /* data before this offset (from data_offset) known to
                             be in sync */
