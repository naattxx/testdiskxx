...
static auto fat_find_info(disk_t &disk_car, unsigned int *reserved, unsigned int *fat_length,
                          const partition_t &partition, const uint64_t max_offset, const int p_fat12, const int p_fat16,
                          const int p_fat32, const int verbose, const int dump_ind, const unsigned int expert,
                          unsigned int *fats) -> upart_type_t;
static auto fat_find_type(disk_t &disk_car, const partition_t &partition, const uint64_t max_offset, const int p_fat12,
                          const int p_fat16, const int p_fat32, const int verbose, const int dump_ind,
                          unsigned int *nbr_offset, info_offset_t *info_offset, const unsigned int max_nbr_offset)
    -> int;

static auto fat_find_fat_start(const unsigned char *buffer, const int p_fat12, const int p_fat16, const int p_fat32,
                               unsigned long int *fat_offset, const unsigned int sector_size) -> unsigned int;

static void create_fat_boot_sector(disk_t &disk_car, partition_t &partition, const unsigned int reserved,
                                   const int verbose, const unsigned int dir_entries,
                                   const unsigned long int root_cluster, const unsigned int sectors_per_cluster,
                                   const unsigned int fat_length, const upart_type_t upart_type,
                                   const unsigned int fats, char **current_cmd);
static auto fat32_find_root_cluster(disk_t &disk_car, const partition_t &partition,
                                    const unsigned int sectors_per_cluster, const unsigned long int no_of_cluster,
                                    const unsigned int reserved, const unsigned int fat_length, const int verbose,
                                    const unsigned int expert, const unsigned int first_free_cluster,
                                    const unsigned int fats) -> unsigned int;
static auto write_FAT_boot_code_aux(unsigned char *buffer) -> int;
static auto find_dir_entries(disk_t &disk_car, const partition_t &partition, const unsigned int offset,
                             const int verbose) -> int;
static auto analyse_dir_entries(disk_t &disk_car, const partition_t &partition, const unsigned int offset,
                                const int verbose) -> int;
static auto analyse_dir_entries2(disk_t &disk_car, const partition_t &partition, const unsigned int reserved,
                                 const unsigned int fat_length, const int verbose, unsigned int root_size_max,
                                 const upart_type_t upart_type, const unsigned int fats) -> int;
static auto calcul_sectors_per_cluster(const upart_type_t upart_type, const unsigned long int data_size,
                                       const unsigned int fat_length, const unsigned int sector_size) -> int;
static auto check_FAT_dir_entry(const unsigned char *entry, const unsigned int entry_nr) -> int;
static auto get_subdirectory(disk_t &disk_car, const uint64_t hd_offset, const unsigned long int i)
    -> unsigned long int;

#ifdef HAVE_NCURSES
static int fat32_create_rootdir(disk_t &disk_car, const partition_t &partition, const unsigned int reserved,
                                const unsigned int fat_length, const unsigned int root_cluster,
                                const unsigned int sectors_per_cluster, const int verbose, file_info_t *rootdir_list,
                                const unsigned int fats);
static upart_type_t select_fat_info(const info_offset_t *info_offset, const unsigned int nbr_offset,
                                    unsigned int *reserved, unsigned int *fat_length,
                                    const unsigned long int max_sector_offset, unsigned int *fats);
#endif
...
static auto check_FAT_dir_entry(const unsigned char *entry, const unsigned int entry_nr) -> int
{
    ...
    for (i = 0; i < 8 + 3; i++)
    {
        ...
        if ((car >= 0x06 && car <= 0x1f) || (car >= 0x3a && car <= 0x3f) || (car >= 'a' && car <= 'z'))
            return 2;
        switch (car)
        {
        case 0x1:
...
static auto get_subdirectory(disk_t &disk_car, const uint64_t hd_offset, const unsigned long int i) -> unsigned long int
{
    unsigned char buffer[DEFAULT_SECTOR_SIZE];
    const auto *entry1 = reinterpret_cast<const struct msdos_dir_entry *>(&buffer[0]);
    const auto *entry2 = reinterpret_cast<const struct msdos_dir_entry *>(&buffer[0x20]);
...
static int ask_root_directory(const disk_t &disk_car, const partition_t &partition, const file_info_t *dir_list,
                              const unsigned long int cluster)
{
    ...
    do
    {
        ...
        for (i = 0, file_walker = dir_list->list.next; file_walker != &dir_list->list && i < offset;
             file_walker = file_walker->next, i++)
            ;
        for (i = offset; file_walker != &dir_list->list && (i - offset) < INTER_DIR;
             file_walker = file_walker->next, i++)
        {
            ...
            wprintw(window, "%s %3u %3u ", str, (unsigned int)current_file->st_uid, (unsigned int)current_file->st_gid);
...
static auto is_root_cluster_candidat(const dir_list_t &dir_list) -> int
{
        ...
        return 0;
    {
        ...
        for (root_cluster = 2; (root_cluster < 2 + no_of_cluster) && (ind_stop == 0); root_cluster++)
        {
...
#endif
            if (std::cmp_equal(
                    disk_car.pread(disk_car, buffer, cluster_size,
                                   partition.part_offset +
                                       (start_data + static_cast<uint64_t>(root_cluster - 2) * sectors_per_cluster) *
                    ...
                    cluster_size))
            {
                ...
                if (verbose > 1)
                {
                    ; // log_verbose("fat32_find_root_cluster test cluster={}\n",root_cluster);
                    /*
                ...
                { /* Directory found */
                    ...
                    {
                        dir_list_t dir_list;
                        log_info("First-level directory found at cluster {}\n", root_cluster);
                ...
                { /* Potential root directory */
                    ...
                    for (i = 0; found && i < cluster_size / 0x20; i++)
                    {
                        ...
                        if (verbose > 2)
                            ; // log_verbose("fat32_find_root_cluster root_cluster={} i={} etat=%d
                        ...
                        if ((buffer[i * 0x20] != DELETED_FLAG) &&
                            (buffer[i * 0x20 + 0xB] != ATTR_EXT &&
                             (buffer[i * 0x20 + 0xB] & ATTR_DIR) != 0)) /* Test directory */
                        {
                            nb_subdir++;
                        }
                    }
                    ...
                    {
                        const auto *entry = reinterpret_cast<const struct msdos_dir_entry *>(&buffer[i * 0x20]);
                        if ((buffer[i * 0x20] != DELETED_FLAG) &&
                            (buffer[i * 0x20 + 0xB] != ATTR_EXT &&
                             (buffer[i * 0x20 + 0xB] & ATTR_DIR) != 0)) /* Test directory */
                        {
                            ...
                            if (cluster > 2 + no_of_cluster ||
                                get_subdirectory(
                                    disk_car,
                                    partition.part_offset +
                                        (start_data + static_cast<uint64_t>(cluster - 2) * sectors_per_cluster) *
                                            disk_car.sector_size,
                                    cluster) != 0)
                            {
                                /*	    if(verbose) */
                    ...
                    if (found)
                    {
                        if ((nb_subdir_ok > nb_subdir * 0.90) && (nb_subdir >= 3))
                        {
                            ...
                            if (verbose > 0)
                                ; // log_verbose("cluster {}, etat=%d, found={},nb_subdir=%d,nb_subdir_ok=%d\n",
                            ;     // root_cluster, etat, found, nb_subdir, nb_subdir_ok);
                            for (back = 0; back < 10; back++)
                            {
                                new_root_cluster = tmp;
                                tmp = fat32_get_prev_cluster(disk_car, partition, reserved, new_root_cluster,
                                                             no_of_cluster);
                                if (verbose > 0)
                                ...
                                /* Read the cluster */
                                if (std::cmp_not_equal(disk_car.pread(disk_car, buffer, cluster_size,
                                                                      partition.part_offset +
                                                                          (start_data + static_cast<uint64_t>(tmp - 2) *
                                                                                            sectors_per_cluster) *
                                                                              disk_car.sector_size),
                                                       cluster_size))
                                {
        ...
        if (ind_stop != 0)
            log_info("Search root cluster stopped: %10lu (2..{})\n", root_cluster, no_of_cluster + 1);
        else
        ...
        else
        {
            ...
            if (expert > 0)
            {
                if (ask_confirmation("Create a new root cluster with {} first-level directories (Expert only) (Y/N)",
                                     dir_nbr) != 0 &&
                    ask_confirmation("Write root cluster, confirm ? (Y/N)") != 0)
                {
                    root_cluster = first_free_cluster;
                    fat32_create_rootdir(disk_car, partition, reserved, fat_length, root_cluster, sectors_per_cluster,
                                         verbose, &rootdir_list, fats);
                }
            }
#endif
...
static void fat_date_unix2dos(time_t unix_date, unsigned short *mstime, unsigned short *msdate)
{
    ...
    *mstime = le16((unix_date % 60) / 2 + (((unix_date / 60) % 60) << 5) + (((unix_date / 3600) % 24) << 11));
                                ...
                                const unsigned int fats)
{
    ...
    if (verbose > 0)
    {
        log_trace("fat32_create_rootdir(reserved={},fat_length={},root_cluster={},sectors_per_cluster={})\n", reserved,
                  fat_length, root_cluster, sectors_per_cluster);
    }
    ...
    td_list_for_each(file_walker, &rootdir_list->list)
    {
        ...
        if (++current_entry == (cluster_size / sizeof(struct msdos_dir_entry)))
        {
            ...
            if ((unsigned)disk_car->pwrite(disk_car, buffer, cluster_size,
                                           partition.part_offset +
                                               (start_data + (uint64_t)(cluster - 2) * sectors_per_cluster) *
                             ...
                             const int verbose) -> int
{
    ...
    for (i = 0, hd_offset = partition.part_offset + static_cast<uint64_t>(offset) * disk_car.sector_size;
         i < 200 && i < offset; i++, hd_offset -= disk_car.sector_size)
    {
        if (std::cmp_not_equal(disk_car.pread(disk_car, buffer, disk_car.sector_size, hd_offset), disk_car.sector_size))
        {
            log_error("dir_entries: read error, dir_entries>={} ({} sectors)\n", i * (disk_car.sector_size / 32), i);
        }
        else
        {
            unsigned int j;
            ...
            for (j = disk_car.sector_size / 32 - 1; j > 0; j--)
            {
                if (verbose > 1)
                {
                    // log_verbose("find_dir_entries sector={} entry={} dir_entry_found=%d\n",
                                ...
                                const int verbose) -> int
{
    ...
    uint64_t hd_offset;
    auto *buffer = new unsigned char[disk_car.sector_size];
    hd_offset = partition.part_offset + static_cast<uint64_t>(offset) * disk_car.sector_size;
    for (i = 0; i < 200; i++)
    {
        if (std::cmp_not_equal(disk_car.pread(disk_car, buffer, disk_car.sector_size, hd_offset), disk_car.sector_size))
        {
                                 ...
                                 const upart_type_t upart_type, const unsigned int fats) -> int
{
    ...
    buffer_dir = new unsigned char[root_dir_size];
    if (std::cmp_not_equal(disk_car.pread(disk_car, buffer_dir, root_dir_size,
                                          partition.part_offset + static_cast<uint64_t>(reserved + fats * fat_length) *
                                                                      disk_car.sector_size),
                           root_dir_size))
    {
    ...
    for (const file_info_t &current_file : dir_list)
    {
        if (LINUX_S_ISDIR(current_file.st_mode) && (current_file.status & FILE_STATUS_DELETED) == 0)
        {
            ...
            {
                const uint64_t start_data =
                    reserved + fats * fat_length +
                    (dir_entries + (disk_car.sector_size / 32) - 1) / (disk_car.sector_size / 32);
                const unsigned int sectors_per_cluster =
                    calcul_sectors_per_cluster(upart_type, partition.part_size / disk_car.sector_size - start_data,
                                               fat_length, disk_car.sector_size);
                if (verbose > 1)
                {
                    ; // log_verbose("dir_entries {}, sectors_per_cluster {}\n",dir_entries,sectors_per_cluster);
                }
                if (std::cmp_equal(
                        disk_car.pread(disk_car, buffer_dir, disk_car.sector_size,
                                       partition.part_offset +
                                           (start_data + static_cast<uint64_t>(new_inode - 2) * sectors_per_cluster) *
                                               disk_car.sector_size),
                        disk_car.sector_size))
                {
                    if (buffer_dir[0] == '.' && is_fat_directory(buffer_dir))
                    {
                        ...
                        const unsigned long int cluster_prev = fat_get_cluster_from_entry(
                            reinterpret_cast<const struct msdos_dir_entry *>(&buffer_dir[0x20]));
                        if (verbose > 1)
                        {
...
#ifdef HAVE_NCURSES
static void fat32_dump_ncurses(disk_t &disk_car, const partition_t &partition, const upart_type_t upart_type,
                               const unsigned char *orgboot, const unsigned char *newboot)
{
    ...
    dump2(window, newboot, orgboot,
          (unsigned int)(upart_type == UP_FAT32 ? 3 * disk_car->sector_size : DEFAULT_SECTOR_SIZE));
    delwin(window);
    (void)clearok(stdscr, TRUE);
#ifdef HAVE_TOUCHWIN
    touchwin(stdscr);
...
static void fat32_dump(disk_t &disk_car, const partition_t &partition, const upart_type_t upart_type,
                       const unsigned char *orgboot, const unsigned char *newboot, char **current_cmd)
{
...
static void menu_write_fat_boot_sector(disk_t &disk_car, partition_t &partition, const int verbose,
                                       const upart_type_t upart_type, const unsigned char *orgboot,
                                       const unsigned char *newboot, const int error, char **current_cmd)
{
    const auto *fat_header = reinterpret_cast<const struct fat_boot_sector *>(newboot);
#ifdef HAVE_NCURSES
    const struct MenuItem menuSaveBoot[] = {{'D', "Dump", "Dump sector"},
    ...
    do
    {
        ...
        {
#ifdef HAVE_NCURSES
            command = wmenuSelect(stdscr, INTER_FATBS_Y + 1, INTER_FATBS_Y, INTER_FATBS_X, menuSaveBoot, 8, options,
                                  MENU_HORIZ | MENU_BUTTON, 1);
#else
            command = 0;
#endif
        }
        ...
        case 'w':
        case 'W':
            if (strchr(options, 'W') != nullptr)
                do_write = 1;
            break;
        case 'd':
        case 'D':
            if (strchr(options, 'D') != nullptr)
                          ...
                          ))
    {
        ...
        {
            if (static_cast<unsigned>(disk_car.pwrite(disk_car, newboot, 3 * disk_car.sector_size,
                                                      partition.part_offset)) != 3 * disk_car.sector_size ||
                static_cast<unsigned>(
                    disk_car.pwrite(disk_car, newboot, 3 * disk_car.sector_size,
                                    partition.part_offset + static_cast<uint64_t> le16(fat_header->backup_boot) *
                                                                disk_car.sector_size)) != 3 * disk_car.sector_size)
                err = 1;
        }
        else
        {
                                   ...
                                   const unsigned int fats, char **current_cmd)
{
    ...
    orgboot = new unsigned char[3 * disk_car.sector_size];
    newboot = new unsigned char[3 * disk_car.sector_size];
    org_fat_header = reinterpret_cast<const struct fat_boot_sector *>(orgboot);
    fat_header = reinterpret_cast<struct fat_boot_sector *>(newboot);
    if (static_cast<unsigned>(disk_car.pread(disk_car, orgboot, 3 * disk_car.sector_size, partition.part_offset)) !=
    ...
    if (memcmp(fat_header->system_id, "MSDOS5.0", 8) && memcmp(fat_header->system_id, "MSWIN4.0", 8) &&
  ...
  if(partition.status==STATUS_LOG)
    ...
    {
    case UP_FAT12:
        part_size = le16(fat_header->reserved) + fats * fat_length + dir_entries * 32 / disk_car.sector_size +
    ...
    case UP_FAT16:
        ...
        break;
    case UP_FAT32:
        part_size = le16(fat_header->reserved) + fats * fat_length +
    ...
    case UP_FAT12:
        ...
        if (check_VFAT_volume_name(reinterpret_cast<const char *>(&newboot[FAT1X_PART_NAME]), 11))
    ...
    case UP_FAT32:
       ...
       Bits 8-15 -- Reserved.
        ...
        if (check_VFAT_volume_name(reinterpret_cast<const char *>(&newboot[FAT32_PART_NAME]), 11))
...
/*@
  ...
  @ assigns \nothing;
  @*/
static auto up2power_aux(const unsigned int number) -> unsigned int
{
    if (number == 0)
        return 0;
    else
...
/*@
  @ assigns \nothing;
  @*/
static auto up2power(const unsigned int number) -> unsigned int
{
    if (number == 0)
        return 1;
...
static auto calcul_sectors_per_cluster(const upart_type_t upart_type, const unsigned long int data_size,
                                       const unsigned int fat_length, const unsigned int sector_size) -> int
{
    /* log_info("calcul_sectors_per_cluster data_size={}, fat_length={},
                               ...
                               unsigned long int *fat_offset, const unsigned int sector_size) -> unsigned int
{
    ...
    info_offset = reinterpret_cast<info_offset_t *>(new unsigned char[sector_size * sizeof(info_offset_t)]);
    if (p_fat12 != 0)
    {
        unsigned int i;
        unsigned int low;
        ...
        while (high < (sector_size - 1))
        {
            ...
            if ((cluster != 0) && ((cluster & 0x0ff8) != static_cast<unsigned>(0x0ff8)) &&
                (((cluster - i - 1) * 3) % (2 * sector_size) == 0))
            {
                unsigned int j;
                for (j = 0; (j < nbr_offset) && (info_offset[j].offset != (cluster - i - 1) * 3 / (2 * sector_size) ||
                                                 info_offset[j].fat_type != 12);
                     j++)
                    ;
                if (j < nbr_offset)
                    info_offset[j].nbr++;
        ...
        while (high < (sector_size - 1))
        {
            ...
            if (low == 0)
                ...
                cluster = (buffer[high + 1] << 4) | ((buffer[high] & 0xF0) >> 4);
            if ((cluster != 0) && ((cluster & 0x0ff8) != static_cast<unsigned>(0x0ff8)) &&
                (((cluster - i - 1) * 3 + 1) % (2 * sector_size) == 0))
            {
                unsigned int j;
                for (j = 0;
                     (j < nbr_offset) && (info_offset[j].offset != ((cluster - i - 1) * 3 + 1) / (2 * sector_size) ||
                                          info_offset[j].fat_type != 12);
                     j++)
                    ;
                if (j < nbr_offset)
        ...
        while (high < (sector_size - 1))
        {
            ...
            if (low == 0)
                ...
                cluster = (buffer[high + 1] << 4) | ((buffer[high] & 0xF0) >> 4);
            if ((cluster != 0) && ((cluster & 0x0ff8) != static_cast<unsigned>(0x0ff8)) &&
                (((cluster - i - 1) * 3 + 2) % (2 * sector_size) == 0))
            {
                unsigned int j;
                for (j = 0;
                     (j < nbr_offset) && (info_offset[j].offset != ((cluster - i - 1) * 3 + 2) / (2 * sector_size) ||
                                          info_offset[j].fat_type != 12);
                     j++)
                    ;
                if (j < nbr_offset)
        ...
        {
            unsigned int j;
            for (j = 0; (j < nbr_offset) && (info_offset[j].offset != 0 || info_offset[j].fat_type != 12); j++)
                ;
            if (j < nbr_offset)
                info_offset[j].nbr++;
            else
    ...
    {
        unsigned int i, j;
        const auto *p16 = reinterpret_cast<const uint16_t *>(buffer);
        unsigned int err = 0;
        for (i = 0; (i < sector_size / 2) && (err == 0); i++)
        {
            ...
            if ((cluster != 0) && ((cluster & 0x0fff8) != static_cast<unsigned>(0x0fff8)))
            {
        ...
        if (err == 0)
        {
            for (i = 0; i < sector_size / 2; i++)
            {
                ...
                {
                    for (j = 0; (j < nbr_offset) && (info_offset[j].offset != (cluster - i - 1) / (sector_size / 2) ||
                                                     info_offset[j].fat_type != 16);
                         j++)
                        ;
                    if (j < nbr_offset)
                        info_offset[j].nbr++;
        ...
        if ((buffer[0] == 0xF0 || buffer[0] >= 0xF8) && buffer[1] == 0xFF && buffer[2] == 0xFF &&
            ((buffer[3] & 0xF7) == 0xF7))
        {
    ...
    if (p_fat32 != 0)
    {
        ...
        for (i = 0; (i < sector_size / 4) && (err == 0); i++)
        {
            ...
            if ((cluster != 0) && ((cluster & 0x0ffffff8) != static_cast<unsigned>(0x0ffffff8)))
            {
        ...
        if (err == 0)
        {
            for (i = 0; i < sector_size / 4; i++)
            {
                ...
                {
                    for (j = 0; (j < nbr_offset) && ((info_offset[j].offset != (cluster - i - 1) / (sector_size / 4)) ||
                                                     (info_offset[j].fat_type != 32));
                         j++)
                        ;
                    if (j < nbr_offset)
                        info_offset[j].nbr++;
        ...
        if ((buffer[0] == 0xF0 || buffer[0] >= 0xF8) && buffer[1] == 0xFF && buffer[2] == 0xFF &&
            ((buffer[3] == 0x0F) || (buffer[3] == 0xFF)) && buffer[4] == 0xFF && buffer[5] == 0xFF && buffer[6] == 0xFF)
        {
    ...
    -> int
{
    ...
    if (verbose > 0)
    {
        ; // log_trace("fat_find_type(max_offset={}, p_fat12=%d, p_fat16=%d, p_fat32=%d, debug=%d, dump_ind=%d)\n",
    ...
    wattroff(stdscr, A_REVERSE);
#endif
    for (offset = disk_car.sector_size; offset < max_offset && !ind_stop; offset += disk_car.sector_size)
    {
#ifdef HAVE_NCURSES
                           ...
                           disk_car.sector_size))
        {
            ...
            const unsigned int fat_type =
                fat_find_fat_start(buffer, p_fat12, p_fat16, p_fat32, &fat_offset, disk_car.sector_size);
            if (fat_type != 0 && fat_offset <= (offset / disk_car.sector_size))
            {
                unsigned int j;
                if (verbose > 1)
                {
                    log_info("fat_find_fat_start FAT{} at {}:{}\n", fat_type,
                          ...
                          unsigned int *fats) -> upart_type_t
{
    ...
    fat_find_type(disk_car, partition, max_offset, p_fat12, p_fat16, p_fat32, verbose, dump_ind, &nbr_offset,
                  &info_offset[0], 0x400);
    for (i = 0; i < nbr_offset; i++)
    {
        const uint64_t end =
            partition.part_offset + static_cast<uint64_t>(info_offset[i].offset) * disk_car.sector_size;
        log_info("FAT{} at {}({}/{}/{}), nbr={}\n", info_offset[i].fat_type, info_offset[i].offset,
                 offset2cylinder(disk_car, end), offset2head(disk_car, end), offset2sector(disk_car, end),
                 info_offset[i].nbr);
#ifdef HAVE_NCURSES
        if (dump_ind > 0)
        {
                                    ...
                                    const unsigned long int max_sector_offset, unsigned int *fats)
{
    ...
    const struct MenuItem menuSelectFAT[] = {
        {'P', "Previous", ""}, {'N', "Next", ""}, {'Q', "Proceed", "Set FAT table location"}, {0, NULL, NULL}};
    screen_buffer_reset();
...
auto rebuild_FAT_BS(disk_t &disk_car, partition_t &partition, const int verbose, const int dump_ind,
                    const unsigned int expert, char **current_cmd) -> int
{
     ...
     * */
    if (partition.part_size > static_cast<uint64_t>(2 * 1024 + 1) * 1024 * 1024)
    {
        p_fat32 = 1;
        p_fat16 = 0;
        p_fat12 = 0;
        ...
        if (partition.part_size >= static_cast<uint64_t>(1 + 2 * 13 + 32768 + 63) * 512)
        {
            p_fat32 = 1;
            p_fat16 = 1;
            p_fat12 = 0;
    ...
    wrefresh(stdscr);
#endif
    upart_type = fat_find_info(disk_car, &reserved, &fat_length, partition, max_offset, p_fat12, p_fat16, p_fat32,
                               verbose, dump_ind, expert, &fats);
#ifdef HAVE_NCURSES
    aff_copy(stdscr);
    wmove(stdscr, 4, 0);
    wprintw(stdscr, "%s", disk_car->description(disk_car));
    ...
    if (verbose > 0)
    {
        ...
        log_info(", reserved={}, fat_length={}\n", reserved, fat_length);
    }
    if ((upart_type != UP_FAT12 && upart_type != UP_FAT16 && upart_type != UP_FAT32) || (fat_length == 0) ||
        (reserved == 0))
    {
        uint64_t start_data = 0;
        if (find_sectors_per_cluster(disk_car, partition, verbose, dump_ind, &sectors_per_cluster, &start_data,
                                     upart_type) == 0)
        {
        ...
        upart_type =
            no_of_cluster2part_type((partition.part_size / disk_car.sector_size - start_data) / sectors_per_cluster);
        switch (upart_type)
        {
        case UP_FAT12:
            log_info("FAT : 12\n");
        ...
        case UP_FAT16:
            ...
            case 512:
                ...
                break;
            default:
                log_warning("dir_entries: {} (unusual value)\n", dir_entries);
                break;
            }
    ...
    if (upart_type && (fat_length > 1))
    {
        ...
        if ((upart_type == UP_FAT12) || (upart_type == UP_FAT16))
        {
            ...
            log_info("dir_entries {}\n", dir_entries);
            dir_entries =
                analyse_dir_entries2(disk_car, partition, reserved, fat_length, verbose, dir_entries, upart_type, fats);
            log_info("dir_entries {}\n", dir_entries);
            if (dir_entries == 0)
            {
        ...
        {
            log_error("Error part_size={} <= start_data={}\n",
        ...
        {
            /* Use first fat */
            fat32_free_info(disk_car, partition, reserved, data_size / sectors_per_cluster, &first_free_cluster,
                            &free_cluster_count);
            /* FAT32 : Find root cluster */
            root_cluster =
                fat32_find_root_cluster(disk_car, partition, sectors_per_cluster, data_size / sectors_per_cluster,
                                        reserved, fat_length, verbose, expert, first_free_cluster, fats);
            if (expert > 0)
            {
#ifdef HAVE_NCURSES
                int i;
        ...
        wrefresh(stdscr);
#endif
        create_fat_boot_sector(disk_car, partition, reserved, verbose, dir_entries, root_cluster, sectors_per_cluster,
                               fat_length, upart_type, fats, current_cmd);
        if (verbose)
        {
            log_info("\n");
            log_partition(disk_car, partition);
...
auto FAT_init_rootdir(disk_t &disk_car, partition_t &partition, const int verbose, char **current_cmd) -> int
{
    ...
    fat_header = reinterpret_cast<struct fat_boot_sector *>(buffer);
    ...
    for (sector = start_rootdir; error == 0 && sector < start_data; sector++)
    {
        if (std::cmp_not_equal(
                disk_car.pread(disk_car, buffer, disk_car.sector_size,
                               partition.part_offset + static_cast<uint64_t>(sector) * disk_car.sector_size),
                disk_car.sector_size))
        {
            log_error("FAT_init_rootdir: read error at sector {}\n", sector);
        }
        else
        {
            unsigned int i;
    ...
    if (ask_confirmation("Initialize FAT root directory, confirm ? (Y/N)") != 0)
    {
        ...
        for (sector = start_rootdir; sector < start_data; sector++)
        {
            if ((unsigned)disk_car->pwrite(disk_car, buffer, disk_car->sector_size,
                                           partition.part_offset + (uint64_t)sector * disk_car->sector_size) !=
                disk_car->sector_size)
            {
                err = 1;
            }
...
auto repair_FAT_table(disk_t &disk_car, partition_t &partition, const int verbose, char **current_cmd) -> int
{
    if (check_FAT(disk_car, partition, verbose) != 0)
    {
        // display_message("Boot sector not valid, can't check FAT.\n");
    ...
    }
    {
...
#endif
        {
            ...
            fat_length =
                le16(fat_header->fat_length) > 0 ? le16(fat_header->fat_length) : le32(fat_header->fat32_length);
            ...
            start_data = start_fat1 + fats * fat_length +
        ...
        }
        {
            const unsigned int buffer_size =
                (partition.upart_type == UP_FAT12 ? 2 * disk_car.sector_size : disk_car.sector_size);
            ...
            for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                buffer_fat[fat_nbr] = new unsigned char[fats * buffer_size];
            for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
            {
            ...
            for (cluster = 2; cluster <= no_of_cluster + 1; cluster++)
            {
                ...
                if (offset_s != old_offset_s)
                {
                    ...
                    /* Write if necessary */
                    {
                                ...
                                for (fat_nbr = 1; fat_nbr < fats; fat_nbr++)
                                {
                                    ...
                                    else if (fat_history[fat_nbr][FAT_OK] == fat_history[good_fat_nbr][FAT_OK])
                                    {
                                        ...
                                        if (fat_find_fat_start(buffer_fat[fat_nbr], (partition.upart_type == UP_FAT12),
                                                               ...
                                                               (partition.upart_type == UP_FAT32), &fat_offset,
                        ...
                        {
                            // log_verbose("nbr_fat_unreadable {}, nbr_fat_corrupted {}, nbr_fat_ok {}, good_fat_nbr {},
                            // fat_mismatch {}\n",
                        ...
                        if (fat_mismatch != 0)
                        {
                            ...
                            if (nbr_fat_ok >= 1)
                            {
                                ...
                                for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                                {
                                    if (fat_nbr != good_fat_nbr)
                                    {
                                        ...
                                        {
#ifdef HAVE_NCURSES
                                            if (ask_confirmation("Use FAT{} to repair FAT{} table, confirm ? (Y/N)",
                                                                 good_fat_nbr + 1, fat_nbr + 1) != 0)
                                            {
...
#endif
                                            {
                                                ...
                                                log_info("repair_FAT_table: doesn't correct FAT{} (sector {}) using "
                                                         "FAT{}\n",
                                                         fat_nbr + 1, start_fat1 + fat_length * fat_nbr + old_offset_s,
                                                         good_fat_nbr + 1);
                                            }
                                        }
                                        if (allow_write[fat_nbr] == FAT_REPAIR_YES)
                                        {
                                            log_info("repair_FAT_table: correcting FAT{} (sector {}) using FAT{}\n",
                                                     fat_nbr + 1, start_fat1 + fat_length * fat_nbr + old_offset_s,
                                                     good_fat_nbr + 1);
                                            if (std::cmp_not_equal(
                                                    disk_car.pwrite(disk_car, buffer_fat[good_fat_nbr], rw_size,
                                                                    partition.part_offset +
                            ...
                            else
                            {
                                for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                                {
                                    ...
                                    {
#ifdef HAVE_NCURSES
                                        if (ask_confirmation("Remove invalid cluster from FAT{} table, confirm ? (Y/N)",
                                                             fat_nbr + 1) != 0)
                                        {
                                            allow_write[fat_nbr] = FAT_REPAIR_YES;
                                        }
                                        ...
                                        {
                                            allow_write[fat_nbr] = FAT_REPAIR_NO;
                                            log_info("repair_FAT_table: doesn't correct FAT{} (sector {})\n",
                                                     fat_nbr + 1, start_fat1 + fat_length * fat_nbr + old_offset_s);
                                        }
                                    }
                                    if (allow_write[fat_nbr] == FAT_REPAIR_YES)
                                    {
                                        log_info("repair_FAT_table: correcting FAT{} (sector {})\n", fat_nbr + 1,
                    ...
                    for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                    {
                        ...
                        if (verbose > 1)
                        {
                            log_info("repair_FAT_table: read sector {} (FAT{})\n",
                                     (start_fat1 + fat_length * fat_nbr + offset_s), fat_nbr + 1);
                        }
                        if (std::cmp_not_equal(
                                disk_car.pread(disk_car, buffer_fat[fat_nbr], rw_size,
                                               partition.part_offset +
                                                   static_cast<uint64_t>(start_fat1 + fat_length * fat_nbr + offset_s) *
                                                       disk_car.sector_size),
                                rw_size))
                        {
                            log_error("repair_FAT_table: read error sector {}\n",
                ...
                for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                {
                    if (partition.upart_type == UP_FAT32)
                    {
                        ...
                        {
#ifdef DEBUG
                            log_trace("FAT{} cluster {}(%lx)->{}(%lx)\n", fat_nbr + 1, cluster, cluster, next_cluster,
                                      next_cluster);
#endif
                    ...
                    else
                    {
                        ...
                        {
                            if ((cluster & 1) != 0)
                                *(reinterpret_cast<uint16_t *>(&buffer_fat[fat_nbr][offset_o])) = le16(
                                    (FAT12_EOC << 4) | (le16((*((uint16_t *)&buffer_fat[fat_nbr][offset_o]))) & 0x0F));
                            else
                                *(reinterpret_cast<uint16_t *>(&buffer_fat[fat_nbr][offset_o])) =
                                    le16(FAT12_EOC | (le16((*((uint16_t *)&buffer_fat[fat_nbr][offset_o]))) & 0xF000));
                            fat_status[fat_nbr] = FAT_CORRUPTED;
                        }
                    }
                }
            ...
            /* Write if necessary the last cluster */
            {
                ...
                if (verbose > 1 || nbr_fat_ok != fats || fat_mismatch > 0)
                {
                    log_info("nbr_fat_unreadable {}, nbr_fat_corrupted {}, nbr_fat_ok {}, good_fat_nbr {}, "
                             "fat_mismatch {}\n",
                             nbr_fat_unreadable, nbr_fat_corrupted, nbr_fat_ok, good_fat_nbr, fat_mismatch);
                }
                /* Write FAT if necessary */
                if (fat_mismatch != 0)
                {
                    ...
                    if (nbr_fat_ok >= 1)
                    {
                        ...
                        for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                        {
                            if (fat_nbr != good_fat_nbr)
                            {
                                ...
                                {
#ifdef HAVE_NCURSES
                                    if (ask_confirmation("Use FAT%u to repair FAT%u table, confirm ? (Y/N)",
                                                         good_fat_nbr + 1, fat_nbr + 1) != 0)
                                    {
                                        allow_write[fat_nbr] = FAT_REPAIR_YES;
                                        log_info("repair_FAT_table: correcting FAT%u (sector {}) using FAT%u\n",
                                                 fat_nbr + 1, start_fat1 + fat_length * fat_nbr + old_offset_s,
                                                 good_fat_nbr + 1);
                                    }
                                    else
                                    ...
                                    {
                                        allow_write[fat_nbr] = FAT_REPAIR_NO;
                                        log_info("repair_FAT_table: doesn't correct FAT{} (sector {}) using FAT{}\n",
                                                 fat_nbr + 1, start_fat1 + fat_length * fat_nbr + old_offset_s,
                                                 good_fat_nbr + 1);
                                    }
                                }
                    ...
                    else
                    {
                        for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                        {
                            ...
                            {
#ifdef HAVE_NCURSES
                                if (ask_confirmation("Remove invalid cluster from FAT%u table, confirm ? (Y/N)",
                                                     fat_nbr + 1) != 0)
                                {
                                    allow_write[fat_nbr] = FAT_REPAIR_YES;
                                    log_info("repair_FAT_table: correcting FAT%u (sector {})\n", fat_nbr + 1,
                                             start_fat1 + fat_length * fat_nbr + old_offset_s);
                                }
                                else
#endif
                            ...
                            if (allow_write[fat_nbr] == FAT_REPAIR_YES)
                            {
                                if (std::cmp_not_equal(
                                        disk_car.pwrite(disk_car, buffer_fat[fat_nbr], rw_size,
                                                        partition.part_offset +
                                                            static_cast<uint64_t>(start_fat1 + fat_length * fat_nbr +
                                                                                  old_offset_s) *
                                                                disk_car.sector_size),
                                        rw_size))
                                {
...
static auto write_FAT_boot_code_aux(unsigned char *buffer) -> int
{
    const unsigned char boot_code[DEFAULT_SECTOR_SIZE] = {
        0xeb, 0x3c, 0x90, 0x6d, 0x6b, 0x64, 0x6f, 0x73, 0x66, 0x73, 0x00, 0x00, 0x02, 0x08, 0x01, 0x00, 0x02, 0x00,
        0x02, 0x00, 0x00, 0xf8, 0xcc, 0x00, 0x3f, 0x00, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x29, 0xf8, 0x3f, 0x7c, 0x3e, 'T',  'E',  'S',  'T',  'D',  'I',  'S',  'K',  0x20, 0x20, 0x20,
        0x46, 0x41, 0x54, 0x31, 0x36, 0x20, 0x20, 0x20, 0x0e, 0x1f, 0xbe, 0x5b, 0x7c, 0xac, 0x22, 0xc0, 0x74, 0x0b,
        0x56, 0xb4, 0x0e, 0xbb, 0x07, 0x00, 0xcd, 0x10, 0x5e, 0xeb, 0xf0, 0x32, 0xe4, 0xcd, 0x16, 0xcd, 0x19, 0xeb,
        0xfe, 'T',  'h',  'i',  's',  ' ',  'i',  's',  ' ',  'n',  'o',  't',  ' ',  'a',  ' ',  'b',  'o',  'o',
        't',  'a',  'b',  'l',  'e',  ' ',  'd',  'i',  's',  'k',  '.',  ' ',  ' ',  'P',  'l',  'e',  'a',  's',
        'e',  ' ',  'i',  'n',  's',  'e',  'r',  't',  ' ',  'a',  ' ',  'b',  'o',  'o',  't',  'a',  'b',  'l',
        'e',  ' ',  'f',  'l',  'o',  'p',  'p',  'y',  ' ',  'a',  'n',  'd',  0x0d, 0x0a, 'p',  'r',  'e',  's',
        's',  ' ',  'a',  'n',  'y',  ' ',  'k',  'e',  'y',  ' ',  't',  'o',  ' ',  't',  'r',  'y',  ' ',  'a',
        'g',  'a',  'i',  'n',  ' ',  '.',  '.',  '.',  ' ',  0x0d, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
