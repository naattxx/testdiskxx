...
static auto fat_find_info(disk_t &disk_car, unsigned int *reserved,
                          unsigned int *fat_length,
                          const partition_t &partition,
                          const uint64_t max_offset, const int p_fat12,
                          const int p_fat16, const int p_fat32,
                          const int verbose, const int dump_ind,
                          const unsigned int expert, unsigned int *fats)
    -> upart_type_t;
static auto fat_find_type(disk_t &disk_car, const partition_t &partition,
                          const uint64_t max_offset, const int p_fat12,
                          const int p_fat16, const int p_fat32,
                          const int verbose, const int dump_ind,
                          unsigned int *nbr_offset, info_offset_t *info_offset,
                          const unsigned int max_nbr_offset) -> int;

static auto fat_find_fat_start(const unsigned char *buffer, const int p_fat12,
                               const int p_fat16, const int p_fat32,
                               unsigned long int *fat_offset,
                               const unsigned int sector_size) -> unsigned int;

static void create_fat_boot_sector(disk_t &disk_car, partition_t &partition,
                                   const unsigned int reserved,
                                   const int verbose,
                                   const unsigned int dir_entries,
                                   const unsigned long int root_cluster,
                                   const unsigned int sectors_per_cluster,
                                   const unsigned int fat_length,
                                   const upart_type_t upart_type,
                                   const unsigned int fats, char **current_cmd);
static auto fat32_find_root_cluster(
    disk_t &disk_car, const partition_t &partition,
    const unsigned int sectors_per_cluster,
    const unsigned long int no_of_cluster, const unsigned int reserved,
    const unsigned int fat_length, const int verbose, const unsigned int expert,
    const unsigned int first_free_cluster, const unsigned int fats
) -> unsigned int;
static auto write_FAT_boot_code_aux(unsigned char *buffer) -> int;
static auto find_dir_entries(disk_t &disk_car, const partition_t &partition,
                             const unsigned int offset, const int verbose)
    -> int;
static auto analyse_dir_entries(disk_t &disk_car, const partition_t &partition,
                                const unsigned int offset, const int verbose)
    -> int;
static auto analyse_dir_entries2(disk_t &disk_car, const partition_t &partition,
                                 const unsigned int reserved,
                                 const unsigned int fat_length,
                                 const int verbose, unsigned int root_size_max,
                                 const upart_type_t upart_type,
                                 const unsigned int fats) -> int;
static auto calcul_sectors_per_cluster(const upart_type_t upart_type,
                                       const unsigned long int data_size,
                                       const unsigned int fat_length,
                                       const unsigned int sector_size) -> int;
static auto check_FAT_dir_entry(const unsigned char *entry,
                                const unsigned int entry_nr) -> int;
static auto get_subdirectory(disk_t &disk_car, const uint64_t hd_offset,
                             const unsigned long int i) -> unsigned long int;

#ifdef HAVE_NCURSES
static int fat32_create_rootdir(disk_t &disk_car, const partition_t &partition,
                                const unsigned int reserved,
                                const unsigned int fat_length,
                                const unsigned int root_cluster,
                                const unsigned int sectors_per_cluster,
                                const int verbose, file_info_t *rootdir_list,
                                const unsigned int fats);
static upart_type_t select_fat_info(const info_offset_t *info_offset,
                                    const unsigned int nbr_offset,
                                    unsigned int *reserved,
                                    unsigned int *fat_length,
                                    const unsigned long int max_sector_offset,
                                    unsigned int *fats);
#endif
                                ...
                                const unsigned int entry_nr) -> int
{
  ...
  for (i = 0; i < 8 + 3; i++)
  {
    ...
    if ((car >= 0x06 && car <= 0x1f) || (car >= 0x3a && car <= 0x3f) ||
        (car >= 'a' && car <= 'z'))
      return 2;
    switch (car)
    {
                             ...
                             const unsigned long int i) -> unsigned long int
{
  ...
  const auto *entry1 =
      reinterpret_cast<const struct msdos_dir_entry *>(&buffer[0]);
  const auto *entry2 =
      reinterpret_cast<const struct msdos_dir_entry *>(&buffer[0x20]);
  if (disk_car.pread(disk_car, &buffer, sizeof(buffer), hd_offset) !=
      sizeof(buffer))
  {
...
static int ask_root_directory(const disk_t &disk_car,
                              const partition_t &partition,
                              const file_info_t *dir_list,
                              const unsigned long int cluster)
{
  ...
  wmove(window, 6, 0);
  wprintw(window,
          "Answer Y(es), N(o), Q(uit) or A(bort interactive mode). N or A if "
          "not sure.");
  curs_set(1);
  do
  {
         ...
         file_walker = file_walker->next, i++)
    {
      const file_info_t *current_file =
          td_list_entry_const(file_walker, const file_info_t, list);
      char str[11];
      char datestr[80];
      ...
      wprintw(window, "%s %3u %3u ", str, (unsigned int)current_file->st_uid,
...
static auto is_root_cluster_candidat(const dir_list_t &dir_list) -> int
{
  ...
  // return (!dir_list.empty() && (&file2->list == &dir_list->list ||
  // file1->st_ino != file2->st_ino));
  return (!dir_list.empty() &&
          (dir_list.size() == 1 || file1.st_ino != file2.st_ino));
}
...
) -> unsigned int
{
    ...
    return 0;
  {
...
#endif
    for (root_cluster = 2;
         (root_cluster < 2 + no_of_cluster) && (ind_stop == 0); root_cluster++)
    {
#ifdef HAVE_NCURSES
      const unsigned long int percent =
          root_cluster * 100 / (2 + no_of_cluster);
      if ((root_cluster & 0xfff) == 0)
      {
        wmove(stdscr, 9, 0);
        wclrtoeol(stdscr);
        wprintw(stdscr, "Search root cluster %10lu/%lu %lu%%", root_cluster,
                2 + no_of_cluster, percent);
        wrefresh(stdscr);
        ind_stop |= check_enter_key_or_s(stdscr);
      }
#endif
      if (std::cmp_equal(disk_car.pread(disk_car, buffer, cluster_size,
                         ...
                         cluster_size))
      {
        ...
        if (verbose > 1)
        {
          ; // log_verbose("fat32_find_root_cluster test
            // cluster={}\n",root_cluster);
            /*
        ...
        { /* Potential root directory */
          ...
          for (i = 0; found && i < cluster_size / 0x20; i++)
          {
            ...
            if (verbose > 2)
              ; // log_verbose("fat32_find_root_cluster root_cluster={} i={}
          ...
          if (found)
          {
            if ((nb_subdir_ok > nb_subdir * 0.90) && (nb_subdir >= 3))
            {
              ...
              if (verbose > 0)
                ; // log_verbose("cluster {}, etat=%d,
                  // found={},nb_subdir=%d,nb_subdir_ok=%d\n",
              ;   // root_cluster, etat, found, nb_subdir, nb_subdir_ok);
              for (back = 0; back < 10; back++)
              {
                new_root_cluster = tmp;
                tmp = fat32_get_prev_cluster(disk_car, partition, reserved,
                                             new_root_cluster, no_of_cluster);
                if (verbose > 0)
                  ; // log_verbose("prev
                    // cluster({})=>{}\n",new_root_cluster,tmp);
                if (tmp == 0)
            ...
            }
            {
              ...
              if (is_root_cluster_candidat(dir_list))
              {
                ...
                {
                  switch (ask_root_directory(disk_car, partition, &dir_list,
                                             root_cluster))
                  {
                  case c_YES:
                    delete_list_file(&dir_list);
    ...
    else
    {
      ...
      if (expert > 0)
      {
        if (ask_confirmation("Create a new root cluster with {} first-level "
                             "directories (Expert only) (Y/N)",
                             dir_nbr) != 0 &&
            ask_confirmation("Write root cluster, confirm ? (Y/N)") != 0)
        {
          root_cluster = first_free_cluster;
          fat32_create_rootdir(disk_car, partition, reserved, fat_length,
                               root_cluster, sectors_per_cluster, verbose,
                               &rootdir_list, fats);
        }
      }
#endif
...
static void fat_date_unix2dos(time_t unix_date, unsigned short *mstime,
                              unsigned short *msdate)
{
  static const int day_n[] = {0,   31,  59,  90,  120, 151, 181, 212,
                                ...
                                const unsigned int fats)
{
  ...
  {
    log_trace(
        "fat32_create_rootdir(reserved={},fat_length={},root_cluster={},"
        "sectors_per_cluster={})\n",
  ...
  td_list_for_each(file_walker, &rootdir_list->list)
  {
    ...
    if (++current_entry == (cluster_size / sizeof(struct msdos_dir_entry)))
    {
      ...
      if (set_next_cluster(disk_car, partition, UP_FAT32, reserved, cluster,
                           next_cluster))
      {
        display_message("Can't modify the FAT entries.\n");
      }
      if (set_next_cluster(disk_car, partition, UP_FAT32, reserved + fat_length,
    ...
    -> int
{
  ...
  for (i = 0, hd_offset = partition.part_offset +
                          static_cast<uint64_t>(offset) * disk_car.sector_size;
       i < 200 && i < offset; i++, hd_offset -= disk_car.sector_size)
  {
    if (std::cmp_not_equal(disk_car.pread(disk_car, buffer,
                                          disk_car.sector_size, hd_offset),
                           disk_car.sector_size))
    {
    ...
    else
    {
      ...
      for (j = disk_car.sector_size / 32 - 1; j > 0; j--)
      {
        if (verbose > 1)
        {
          // log_verbose("find_dir_entries sector={} entry={}
    ...
    -> int
{
  ...
  uint64_t hd_offset;
  auto *buffer = new unsigned char[disk_car.sector_size];
  hd_offset    = partition.part_offset +
                 static_cast<uint64_t>(offset) * disk_car.sector_size;
  for (i = 0; i < 200; i++)
  {
    if (std::cmp_not_equal(disk_car.pread(disk_car, buffer,
                                 ...
                                 const unsigned int fats) -> int
{
  ...
  root_dir_size = (root_size_max * 32 + disk_car.sector_size - 1) /
        ...
        (current_file.status & FILE_STATUS_DELETED) == 0)
    {
           ...
           dir_entries += disk_car.sector_size / 32)
      {
        const uint64_t start_data =
            reserved + fats * fat_length +
            (dir_entries + (disk_car.sector_size / 32) - 1) /
                (disk_car.sector_size / 32);
        const unsigned int sectors_per_cluster = calcul_sectors_per_cluster(
            upart_type, partition.part_size / disk_car.sector_size - start_data,
        ...
        if (std::cmp_equal(
                disk_car.pread(disk_car, buffer_dir, disk_car.sector_size,
                               partition.part_offset +
                                   (start_data +
                                    static_cast<uint64_t>(new_inode - 2) *
                                        sectors_per_cluster) *
                                       disk_car.sector_size),
                disk_car.sector_size
            ))
...
#ifdef HAVE_NCURSES
static void fat32_dump_ncurses(disk_t &disk_car, const partition_t &partition,
                               const upart_type_t upart_type,
                               const unsigned char *orgboot,
                               const unsigned char *newboot)
{
  ...
  dump2(window, newboot, orgboot,
        (unsigned int)(upart_type == UP_FAT32 ? 3 * disk_car->sector_size
                                              : DEFAULT_SECTOR_SIZE));
  delwin(window);
  (void)clearok(stdscr, TRUE);
#ifdef HAVE_TOUCHWIN
...
static void fat32_dump(disk_t &disk_car, const partition_t &partition,
                       const upart_type_t upart_type,
                       const unsigned char *orgboot,
                       const unsigned char *newboot, char **current_cmd)
{
  log_info("     Rebuild Boot sector           Boot sector\n");
  // dump2_log(newboot,orgboot, (unsigned
  // int)(upart_type==UP_FAT32?3*disk_car->sector_size:DEFAULT_SECTOR_SIZE));
...
static void menu_write_fat_boot_sector(disk_t &disk_car, partition_t &partition,
                                       const int verbose,
                                       const upart_type_t upart_type,
                                       const unsigned char *orgboot,
                                       const unsigned char *newboot,
                                       const int error, char **current_cmd)
{
  const auto *fat_header =
      reinterpret_cast<const struct fat_boot_sector *>(newboot);
#ifdef HAVE_NCURSES
  ...
  do
  {
               ...
               DEFAULT_SECTOR_SIZE)) /* Only compare the first sector */
    {
      ...
      const struct fat_boot_sector *org_fat_header =
          (const struct fat_boot_sector *)orgboot;
      dump_2fat_info_ncurses(fat_header, org_fat_header, upart_type,
                             disk_car->sector_size);
      wprintw(
          stdscr,
...
#ifdef HAVE_NCURSES
      command =
          wmenuSelect(stdscr, INTER_FATBS_Y + 1, INTER_FATBS_Y, INTER_FATBS_X,
                      menuSaveBoot, 8, options, MENU_HORIZ | MENU_BUTTON, 1);
#else
      command = 0;
#endif
    }
    ...
    case 'L': {
      ...
      io_redir_add_redir(disk_car, partition.part_offset,
                         3 * disk_car.sector_size, 0, newboot);
                          ...
                          ))
  {
    ...
    {
      if (static_cast<unsigned>(disk_car.pwrite(
              disk_car, newboot, 3 * disk_car.sector_size, partition.part_offset
    ...
    {
      if (disk_car.pwrite(disk_car, newboot, DEFAULT_SECTOR_SIZE,
    ...
    /* Note that TestDisk doesn't repair the filesystem itself, use another
     * utility for that */
  }
  else
                                   ...
                                   const unsigned int fats, char **current_cmd)
{
  ...
  if (static_cast<unsigned>(disk_car.pread(
          disk_car, orgboot, 3 * disk_car.sector_size, partition.part_offset
      )) != 3 * disk_car.sector_size)
  {
  ...
  case UP_FAT12:
    ...
    if (check_VFAT_volume_name(
            reinterpret_cast<const char *>(&newboot[FAT1X_PART_NAME]), 11
        ))
  ...
  case UP_FAT32:
   ...
   Bits 8-15 -- Reserved.
    ...
    if ((memcmp(newboot + FAT_NAME1, "FAT12", 5) == 0) ||
        (memcmp(newboot + FAT_NAME1, "FAT16", 5) == 0))
    ...
    newboot[disk_car.sector_size + 0x1E8] =
        0xFF; /* 488: Free clusters on disk */
    newboot[disk_car.sector_size + 0x1E9] = 0xFF;
    newboot[disk_car.sector_size + 0x1EA] = 0xFF;
    newboot[disk_car.sector_size + 0x1EB] = 0xFF;
    newboot[disk_car.sector_size + 0x1EC] =
        0xFF; /* 492: Next available clusters */
    ...
    newboot[2 * disk_car.sector_size + 0x1FC] =
        0x00; /* End of Sector signature */
    ...
    if (check_VFAT_volume_name(
            reinterpret_cast<const char *>(&newboot[FAT32_PART_NAME]), 11
        ))
                               ...
                               const unsigned int sector_size) -> unsigned int
{
  ...
  info_offset             = reinterpret_cast<info_offset_t *>(
      new unsigned char[sector_size * sizeof(info_offset_t)]
  );
  if (p_fat12 != 0)
  {
    unsigned int i;
    ...
    while (high < (sector_size - 1))
    {
      ...
      if ((cluster != 0) &&
          ((cluster & 0x0ff8) != static_cast<unsigned>(0x0ff8)) &&
          (((cluster - i - 1) * 3) % (2 * sector_size) == 0))
      {
        unsigned int j;
        for (j = 0; (j < nbr_offset) &&
                    (info_offset[j].offset !=
                         (cluster - i - 1) * 3 / (2 * sector_size) ||
    ...
    if ((buffer[0] == 0xF0 || buffer[0] >= 0xF8) && buffer[1] == 0xFF)
    {
      unsigned int j;
      for (j = 0; (j < nbr_offset) &&
                  (info_offset[j].offset != 0 || info_offset[j].fat_type != 12);
           j++)
        ;
      if (j < nbr_offset)
        info_offset[j].nbr++;
  ...
  if (p_fat16 != 0)
  {
    ...
    if (err == 0)
    {
      for (i = 0; i < sector_size / 2; i++)
      {
        ...
        {
          for (j = 0;
               (j < nbr_offset) && (info_offset[j].offset !=
                                        (cluster - i - 1) / (sector_size / 2) ||
    ...
    {
      for (j = 0; (j < nbr_offset) &&
                  (info_offset[j].offset != 0 || info_offset[j].fat_type != 16);
           j++)
        ;
      if (j < nbr_offset)
        info_offset[j].nbr++;
  ...
  if (p_fat32 != 0)
  {
    ...
    {
      for (j = 0; (j < nbr_offset) &&
                  (info_offset[j].offset != 0 || info_offset[j].fat_type != 32);
           j++)
        ;
      if (j < nbr_offset)
        info_offset[j].nbr++;
                          ...
                          const unsigned int max_nbr_offset) -> int
{
  ...
  wattroff(stdscr, A_REVERSE);
#endif
  for (offset = disk_car.sector_size; offset < max_offset && !ind_stop;
       offset += disk_car.sector_size)
  {
                       ...
                       disk_car.sector_size))
    {
      ...
      const unsigned int fat_type =
          fat_find_fat_start(buffer, p_fat12, p_fat16, p_fat32, &fat_offset,
                             disk_car.sector_size);
    ...
    -> upart_type_t
{
  ...
  fat_find_type(disk_car, partition, max_offset, p_fat12, p_fat16, p_fat32,
                verbose, dump_ind, &nbr_offset, &info_offset[0], 0x400);
  for (i = 0; i < nbr_offset; i++)
  {
    const uint64_t end =
        partition.part_offset +
        static_cast<uint64_t>(info_offset[i].offset) * disk_car.sector_size;
    log_info("FAT{} at {}({}/{}/{}), nbr={}\n", info_offset[i].fat_type,
             info_offset[i].offset, offset2cylinder(disk_car, end),
             offset2head(disk_car, end), offset2sector(disk_car, end),
             info_offset[i].nbr);
#ifdef HAVE_NCURSES
    if (dump_ind > 0)
    {
  ...
  else
  {
    ...
    for (i = 0; i < nbr_offset; i++)
    {
      if (info_offset[i].fat_type == info_offset[offset_for_max_nbr].fat_type)
      {
        if (fat_found == 0 &&
            info_offset[i].nbr >=
                (info_offset[offset_for_max_nbr].nbr + 2 - 1) / 2)
        {
          first_fat = i;
          fat_found++;
        }
        else if (fat_found == 1 &&
                 info_offset[i].nbr >= (info_offset[first_fat].nbr + 2 - 1) / 2)
        {
          second_fat = i;
          fat_found++;
        }
    ...
    if (fat_found == 1)
    {
      ...
      case UP_FAT16:
        *reserved = 1;
        ...
        break;
      case UP_FAT32:
        if (info_offset[first_fat].offset == 32 ||
            info_offset[first_fat].offset == 33)
    ...
    else
    {
      ...
      case UP_FAT32:
        *reserved   = info_offset[first_fat].offset;
        *fat_length = info_offset[second_fat].offset - *reserved;
        if (*reserved == 32 || *reserved == 33 ||
            comp_FAT(disk_car, partition, *fat_length, *reserved) == 0)
        {
        }
        else
        {
    ...
    if (verbose > 0)
    {
      log_info("first_fat {}, second_fat {}\n", info_offset[first_fat].offset,
               info_offset[second_fat].offset);
    }
  }
  if (expert > 0)
  {
#ifdef HAVE_NCURSES
    return select_fat_info(info_offset, nbr_offset, reserved, fat_length,
                           max_offset / disk_car->sector_size, fats);
#endif
  }
  return upart_type;
}
                                    ...
                                    unsigned int *fats)
{
  ...
  for (i = 0; i < nbr_offset; i++)
  {
    if (nbr_offset < 30 || info_offset[i].nbr > 1)
      screen_buffer_add(" %02u %8lu   {}\n", info_offset[i].fat_type,
                        info_offset[i].offset, info_offset[i].nbr);
  }
  aff_copy(stdscr);
  wmove(stdscr, 4, 0);
  ...
  *reserved = ask_number(*reserved, 0, max_sector_offset,
                         "FAT1 location (Number of reserved sector) ");
  if (*reserved > 0)
  {
    wmove(stdscr, INTER_FAT_ASK_Y, INTER_FAT_ASK_X);
    fat2_location =
        ask_number(fat2_location, 0, max_sector_offset, "FAT2 location ");
                    ...
                    char **current_cmd) -> int
{
  ...
  if (partition.part_size > static_cast<uint64_t>(2 * 1024 + 1) * 1024 * 1024)
  {
    ...
    if (partition.part_size >=
        static_cast<uint64_t>(1 + 2 * 13 + 32768 + 63) * 512)
    {
      p_fat32 = 1;
      p_fat16 = 1;
      p_fat12 = 0;
  ...
  }
  {
    ...
    fat_length_max =
        fat_length_max / disk_car.sector_size * disk_car.sector_size;
    if (verbose > 1)
    {
  ...
  wrefresh(stdscr);
#endif
  upart_type = fat_find_info(disk_car, &reserved, &fat_length, partition,
                             max_offset, p_fat12, p_fat16, p_fat32, verbose,
                             dump_ind, expert, &fats);
#ifdef HAVE_NCURSES
  aff_copy(stdscr);
  wmove(stdscr, 4, 0);
  wprintw(stdscr, "%s", disk_car->description(disk_car));
      ...
      (fat_length == 0) || (reserved == 0))
  {
    ...
    if (find_sectors_per_cluster(disk_car, partition, verbose, dump_ind,
                                 &sectors_per_cluster, &start_data,
                                 upart_type) == 0)
    {
    ...
    if ((sectors_per_cluster <= 0) ||
        (partition.part_size / disk_car.sector_size <= start_data))
    {
      // display_message("Can't find cluster size\n");
      return 0;
    }
    upart_type = no_of_cluster2part_type(
        (partition.part_size / disk_car.sector_size - start_data) /
        sectors_per_cluster
    );
    switch (upart_type)
    ...
    case UP_FAT16:
      ...
      dir_entries =
          find_dir_entries(disk_car, partition, start_data - 1, verbose);
      switch (dir_entries)
      {
      case 0:
      ...
      fat_length =
          (start_data - reserved - ((dir_entries - 1) / 16 + 1)) / fats;
      break;
    case UP_FAT32:
      if (reserved == 0)
      {
  ...
  if (upart_type && (fat_length > 1))
  {
    ...
    if ((upart_type == UP_FAT12) || (upart_type == UP_FAT16))
    {
      ...
      dir_entries =
          analyse_dir_entries(disk_car, partition, start_data, verbose);
      log_info("dir_entries {}\n", dir_entries);
      dir_entries =
    ...
    /* Get Cluster Size */
    {
      ...
      sectors_per_cluster =
          calcul_sectors_per_cluster(upart_type, data_size, fat_length,
                                     disk_car.sector_size);
      if (verbose > 0)
      ...
      if (expert > 0)
      {
        ...
        wmove(stdscr, INTER_FAT_ASK_Y, INTER_FAT_ASK_X);
        sectors_per_cluster =
            ask_number(sectors_per_cluster, 0, 128, "cluster size ");
#endif
    ...
    {
      /* Use first fat */
      fat32_free_info(disk_car, partition, reserved,
                      data_size / sectors_per_cluster, &first_free_cluster,
                      &free_cluster_count);
      /* FAT32 : Find root cluster */
      root_cluster =
          fat32_find_root_cluster(disk_car, partition, sectors_per_cluster,
                                  data_size / sectors_per_cluster, reserved,
                                  fat_length, verbose, expert,
                                  first_free_cluster, fats);
      if (expert > 0)
      {
#ifdef HAVE_NCURSES
        int i;
        ...
        wmove(stdscr, INTER_FAT_ASK_Y, INTER_FAT_ASK_X);
        root_cluster =
            ask_number(root_cluster, 2, data_size / sectors_per_cluster + 1,
                       "root cluster ");
#endif
        if (verbose > 1)
        {
    ...
    wrefresh(stdscr);
#endif
    create_fat_boot_sector(disk_car, partition, reserved, verbose, dir_entries,
                           root_cluster, sectors_per_cluster, fat_length,
                           upart_type, fats, current_cmd);
    if (verbose)
    {
      log_info("\n");
      log_partition(disk_car, partition);
                      ...
                      const int verbose, char **current_cmd) -> int
{
  ...
  if (ask_confirmation("Initialize FAT root directory, confirm ? (Y/N)") != 0)
  {
    ...
    {
      if ((unsigned)disk_car->pwrite(
              disk_car, buffer, disk_car->sector_size,
  ...
  }
  {
    ...
    }
    {
      const unsigned int buffer_size =
          (partition.upart_type == UP_FAT12 ? 2 * disk_car.sector_size
                                            : disk_car.sector_size);
        ...
        if (offset_s != old_offset_s)
        {
          ...
          /* Write if necessary */
          {
            ...
            if (verbose > 1 || nbr_fat_ok != fats || fat_mismatch > 0)
            {
              ...
              // nbr_fat_ok {}, good_fat_nbr {}, fat_mismatch {}\n",
              //     nbr_fat_unreadable, nbr_fat_corrupted, nbr_fat_ok,
              //     good_fat_nbr, fat_mismatch);
            }
            ...
            if (fat_mismatch != 0)
            {
              ...
              if (nbr_fat_ok >= 1)
              {
                ...
                for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                {
                  if (fat_nbr != good_fat_nbr)
                  {
                    ...
                    {
#ifdef HAVE_NCURSES
                      if (ask_confirmation("Use FAT{} to repair FAT{} table, "
                                           "confirm ? (Y/N)",
...
#endif
                      {
                        ...
                        log_info(
                            "repair_FAT_table: doesn't correct FAT{} (sector "
                            "{}) using "
                            "FAT{}\n",
                            fat_nbr + 1,
                    ...
                    {
                      log_info(
                          "repair_FAT_table: correcting FAT{} (sector {}) "
                          "using FAT{}\n",
                          fat_nbr + 1,
                          start_fat1 + fat_length * fat_nbr + old_offset_s,
                          good_fat_nbr + 1
                      ...
                      if (std::cmp_not_equal(
                              disk_car.pwrite(disk_car,
                                              buffer_fat[good_fat_nbr], rw_size,
                                              partition.part_offset +
                                                  static_cast<uint64_t>(
                                                      start_fat1 +
                                                      fat_length * fat_nbr +
                      ...
                      {
                        ; // display_message("repair_FAT_table: write
                          // failed.\n");
                      }
                    }
                  }
              ...
              else
              {
                for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
                {
                  ...
                  {
#ifdef HAVE_NCURSES
                    if (ask_confirmation("Remove invalid cluster from FAT{} "
                                         "table, confirm ? (Y/N)",
                                         fat_nbr + 1) != 0)
                    {
                      allow_write[fat_nbr] = FAT_REPAIR_YES;
      ...
      /* Write if necessary the last cluster */
      {
        ...
        if (verbose > 1 || nbr_fat_ok != fats || fat_mismatch > 0)
        {
          log_info(
              ...
              "good_fat_nbr {}, "
              "fat_mismatch {}\n",
              nbr_fat_unreadable, nbr_fat_corrupted, nbr_fat_ok, good_fat_nbr,
              fat_mismatch
          );
        }
        /* Write FAT if necessary */
        if (fat_mismatch != 0)
        {
...
#ifdef HAVE_NCURSES
                  if (ask_confirmation(
                          "Use FAT%u to repair FAT%u table, confirm ? (Y/N)",
                          good_fat_nbr + 1, fat_nbr + 1
                      ) != 0)
                  {
                    allow_write[fat_nbr] = FAT_REPAIR_YES;
                    log_info(
                        "repair_FAT_table: correcting FAT%u (sector {}) using "
                        "FAT%u\n",
                        fat_nbr + 1,
                        start_fat1 + fat_length * fat_nbr + old_offset_s,
                        good_fat_nbr + 1
          ...
          else
          {
            for (fat_nbr = 0; fat_nbr < fats; fat_nbr++)
            {
              ...
              {
#ifdef HAVE_NCURSES
                if (ask_confirmation("Remove invalid cluster from FAT%u table, "
                                     "confirm ? (Y/N)",
                                     fat_nbr + 1) != 0)
                {
                  allow_write[fat_nbr] = FAT_REPAIR_YES;
...
static auto write_FAT_boot_code_aux(unsigned char *buffer) -> int
{
  const unsigned char boot_code[DEFAULT_SECTOR_SIZE] = {
      ...
      0x02, 0x08, 0x01, 0x00, 0x02, 0x00, 0x02, 0x00, 0x00, 0xf8, 0xcc, 0x00,
      0x3f, 0x00, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x29, 0xf8, 0x3f, 0x7c, 0x3e, 'T',  'E',  'S',  'T',  'D',
      'I',  'S',  'K',  0x20, 0x20, 0x20, 0x46, 0x41, 0x54, 0x31, 0x36, 0x20,
      0x20, 0x20, 0x0e, 0x1f, 0xbe, 0x5b, 0x7c, 0xac, 0x22, 0xc0, 0x74, 0x0b,
      0x56, 0xb4, 0x0e, 0xbb, 0x07, 0x00, 0xcd, 0x10, 0x5e, 0xeb, 0xf0, 0x32,
      0xe4, 0xcd, 0x16, 0xcd, 0x19, 0xeb, 0xfe, 'T',  'h',  'i',  's',  ' ',
      'i',  's',  ' ',  'n',  'o',  't',  ' ',  'a',  ' ',  'b',  'o',  'o',
      't',  'a',  'b',  'l',  'e',  ' ',  'd',  'i',  's',  'k',  '.',  ' ',
      ' ',  'P',  'l',  'e',  'a',  's',  'e',  ' ',  'i',  'n',  's',  'e',
      'r',  't',  ' ',  'a',  ' ',  'b',  'o',  'o',  't',  'a',  'b',  'l',
      'e',  ' ',  'f',  'l',  'o',  'p',  'p',  'y',  ' ',  'a',  'n',  'd',
      0x0d, 0x0a, 'p',  'r',  'e',  's',  's',  ' ',  'a',  'n',  'y',  ' ',
      'k',  'e',  'y',  ' ',  't',  'o',  ' ',  't',  'r',  'y',  ' ',  'a',
      'g',  'a',  'i',  'n',  ' ',  '.',  '.',  '.',  ' ',  0x0d, 0x0a, 0x00,
