...
auto repair_MFT(disk_t &disk_car, partition_t &partition, const int verbose,
                const unsigned int expert, char **current_cmd) -> int
{
  ...
  ntfs_header = reinterpret_cast<struct ntfs_boot_sector *>(
      new unsigned char[DEFAULT_SECTOR_SIZE]
  );
  ...
  if (std::cmp_not_equal(disk_car.pread(disk_car, buffer_mft,
                                        mftmirr_size_bytes, mft_pos),
                         mftmirr_size_bytes))
  {
  ...
  if (std::cmp_not_equal(disk_car.pread(disk_car, buffer_mftmirr,
                                        mftmirr_size_bytes, mftmirr_pos),
                         mftmirr_size_bytes))
  {
  ...
  {
    log_info(
        "Please quit TestDisk and reboot your computer before trying to fix "
        "the MFT.\n"
    );
  ...
  */
  {
...
#ifdef HAVE_NCURSES
      if (ask_confirmation(
              "Fix MFT using its mirror ? (Y/N) - DANGEROUS NON REVERSIBLE "
              "OPERATION\nUse it ONLY "
              "IF TestDisk and Windows failed to access this filesystem."
          ) != 0)
        use_MFT = 2;
      else
#endif
    ...
    { /* res1==res2 */
      ...
      {
        if (res1 > DIR_PART_OK && res2 > DIR_PART_OK)
          ; // display_message("Both MFT seems ok but they don't match, use
            // chkdsk.\n");
        else
          ; // display_message("MFT and MFT mirror are bad. Failed to repair
            // them.\n");
      }
      else
      {
        ...
        command =
            wmenuSelect_ext(stdscr, 23, INTER_MFT_Y, INTER_MFT_X, menuMFT, 10,
                            "MBQ", MENU_VERT | MENU_VERT_WARN | MENU_BUTTON,
                            &menu, &real_key);
        switch (command)
        {
        case 'b':
  ...
  if (use_MFT == 2)
  {
    if (std::cmp_not_equal(disk_car.pwrite(disk_car, buffer_mftmirr,
                                           mftmirr_size_bytes, mft_pos),
                           mftmirr_size_bytes))
    {
  ...
  {
    if (std::cmp_not_equal(disk_car.pwrite(disk_car, buffer_mft,
                                           mftmirr_size_bytes, mftmirr_pos),
                           mftmirr_size_bytes))
    {
