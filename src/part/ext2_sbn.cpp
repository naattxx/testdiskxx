...
static const uint64_t group_size[4] = {
    ...
    static_cast<uint64_t>(EXT2_MIN_BLOCK_SIZE << 6) * 8 *
...
static auto next_sb(const uint64_t hd_offset_old) -> uint64_t
{
  ...
  for (j = 0; j < 4; j++)
  {
    ...
    {
      uint64_t val;
      for (val = 1; val * group_size[j] + offset <= hd_offset_old;
           val *= factors[i])
        ;
...
#endif
    if (disk_car.pread(disk_car, buffer, 1024,
                       partition.part_offset + hd_offset) == 1024)
    {
      ...
      if (le16(sb->s_magic) == EXT2_SUPER_MAGIC)
      {
        ...
        if (recover_EXT2(disk_car, sb, new_partition, verbose, dump) == 0)
        {
          ...
          wprintw(stdscr,
                  "Ext2 superblock found at sector %llu (block=%llu, "
                  "blocksize=%u)        \n",
