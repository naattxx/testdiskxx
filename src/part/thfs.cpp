...
#ifdef HAVE_NCURSES
static void hfs_dump_ncurses(disk_t &disk_car, const partition_t &partition,
...
static void hfs_dump(disk_t &disk_car, const partition_t &partition,
                     const unsigned char *buffer_bs,
...
static auto HFS_HFSP_boot_sector_command(char **current_cmd,
...
static auto HFS_HFSP_boot_sector_rescan(disk_t &disk_car,
                                        const partition_t &partition,
                                        unsigned char *buffer_bs,
                                        unsigned char *buffer_backup_bs,
                                        const int verbose) -> const char *
{
  int opt_B = 0;
  int opt_O = 0;
#ifdef HAVE_NCURSES
  ...
  if (disk_car.pread(disk_car, buffer_bs, HFSP_BOOT_SECTOR_SIZE,
  ...
  else if (test_HFSP(disk_car,
                     reinterpret_cast<const struct hfsp_vh *>(buffer_bs),
                     partition, verbose, 0) == 0)
  {
    screen_buffer_add("HFS+ OK\n");
    opt_O = 1;
  }
  else if (test_HFS(disk_car, reinterpret_cast<const hfs_mdb_t *>(buffer_bs),
                    partition, verbose, 0) == 0)
  {
    screen_buffer_add("HFS Ok\n");
    opt_O = 1;
  ...
  if (disk_car.pread(disk_car, buffer_backup_bs, HFSP_BOOT_SECTOR_SIZE,
                     partition.part_offset + partition.part_size - 0x400) !=
      HFSP_BOOT_SECTOR_SIZE)
  {
    ...
    memset(buffer_backup_bs, 0, HFSP_BOOT_SECTOR_SIZE);
                     ...
                     reinterpret_cast<const struct hfsp_vh *>(buffer_backup_bs),
                     partition, verbose, 0) == 0)
  {
    screen_buffer_add("HFS+ OK\n");
    opt_B = 1;
  }
  else if (test_HFS(disk_car,
                    reinterpret_cast<const hfs_mdb_t *>(buffer_backup_bs),
                    partition, verbose, 0) == 0)
  {
    screen_buffer_add("HFS Ok\n");
    opt_B = 1;
...
auto HFS_HFSP_boot_sector(disk_t &disk, partition_t &partition,
                          const int verbose, char **current_cmd) -> int
{
  ...
  while (true)
  {
    ...
    if (rescan == 1)
    {
      options = HFS_HFSP_boot_sector_rescan(disk, partition, buffer_bs,
                                            buffer_backup_bs, verbose);
      rescan  = 0;
    }
...
#ifdef HAVE_NCURSES
      if (ask_confirmation("Copy original HFS/HFS+ volume header over backup, "
                           "confirm ? (Y/N)") != 0)
#endif
      {
        ...
        if (disk.pwrite(disk, buffer_bs, HFSP_BOOT_SECTOR_SIZE,
            ...
            HFSP_BOOT_SECTOR_SIZE)
        {
          ; // display_message("Write error: Can't overwrite HFS/HFS+ backup
            // volume header\n");
        }
        disk.sync(disk);
        rescan = 1;
...
#ifdef HAVE_NCURSES
      if (ask_confirmation("Copy backup HFS/HFS+ volume header over main "
                           "volume header, confirm ? (Y/N)") != 0)
#endif
      {
        log_info("copy backup superblock over main superblock\n");
        /* Reset information about backup boot sector */
        partition.sb_offset = 0;
        if (disk.pwrite(disk, buffer_backup_bs, HFSP_BOOT_SECTOR_SIZE,
                        partition.part_offset + 0x400) != HFSP_BOOT_SECTOR_SIZE)
        {
          ; // display_message("Write error: Can't overwrite HFS/HFS+ main
            // volume header\n");
        }
        disk.sync(disk);
        rescan = 1;
