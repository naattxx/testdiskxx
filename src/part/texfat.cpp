...
#ifdef HAVE_NCURSES
static void exFAT_dump_ncurses(disk_t &disk, const partition_t &partition,
                               ...
                               const unsigned char *buffer_backup_bs)
{
  ...
  mvwaddstr(window, 6, 0,
            "Boot sector                       Backup boot record");
...
static void exFAT_dump(disk_t &disk, const partition_t &partition,
                       const unsigned char *buffer_bs,
                       const unsigned char *buffer_backup_bs,
                       char **current_cmd)
{
...
static auto exFAT_boot_sector_command(char **current_cmd, const char *options)
    -> int
{
...
static auto exFAT_boot_sector_rescan(disk_t &disk, const partition_t &partition,
                                     unsigned char *buffer_bs,
                                     unsigned char *buffer_backup_bs) -> const
    char *
{
  ...
  if (disk.pread(disk, buffer_bs, size_bs, partition.part_offset) != size_bs)
  {
    ...
    memset(buffer_bs, 0, size_bs);
               ...
               reinterpret_cast<const struct exfat_super_block *>(buffer_bs)
           ) == 0)
  {
  ...
  screen_buffer_add("\nBackup boot record\n");
  if (disk.pread(disk, buffer_backup_bs, size_bs,
                 partition.part_offset + size_bs) != size_bs)
  {
    screen_buffer_add("Bad: can't read exFAT backup boot record.\n");
    memset(buffer_backup_bs, 0, size_bs);
  }
  else if (test_exFAT(reinterpret_cast<const struct exfat_super_block *>(
               buffer_backup_bs
           )) == 0)
  {
    screen_buffer_add("exFAT OK\n");
...
auto exFAT_boot_sector(disk_t &disk, partition_t &partition, char **current_cmd)
    -> int
{
  unsigned char *buffer_bs;
  ...
  while (true)
  {
    ...
    if (rescan == 1)
    {
      options = exFAT_boot_sector_rescan(disk, partition, buffer_bs,
                                         buffer_backup_bs);
      rescan  = 0;
    }
...
#ifdef HAVE_NCURSES
      if (ask_confirmation(
              "Copy original exFAT boot record over backup, confirm ? (Y/N)"
          ) != 0)
#endif
      {
        log_info("copy original superblock over backup boot\n");
        if (disk.pwrite(disk, buffer_bs, size_bs,
                        partition.part_offset + size_bs) != size_bs)
        {
          ; // display_message("Write error: Can't overwrite exFAT backup boot
            // record\n");
        }
        disk.sync(disk);
        rescan = 1;
...
#ifdef HAVE_NCURSES
      if (ask_confirmation("Copy backup exFAT boot record over main boot "
                           "record, confirm ? (Y/N)") != 0)
#endif
      {
        log_info("copy backup superblock over main superblock\n");
        /* Reset information about backup boot record */
        partition.sb_offset = 0;
        if (disk.pwrite(disk, buffer_backup_bs, size_bs,
                        partition.part_offset) != size_bs)
        {
          ; // display_message("Write error: Can't overwrite exFAT main boot
            // record\n");
        }
        disk.sync(disk);
        rescan = 1;
