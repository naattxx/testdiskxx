...
uint64_t disk_get_size_win32(HANDLE handle, const char *device, const int verbose)
{
    ...
    if (device[0] != '\0' && device[1] != '\0' && device[2] != '\0' && device[3] != '\0' && device[4] != '\0')
    {
        ...
        if (GetDiskFreeSpaceEx(&device[4], (PULARGE_INTEGER)&i64FreeBytesToCaller, (PULARGE_INTEGER)&i64TotalBytes,
                               (PULARGE_INTEGER)&i64FreeBytes) != 0)
        {
            if (verbose > 1)
                log_info("disk_get_size_win32 GetDiskFreeSpaceEx %s: %llu\n", device,
                         (long long unsigned)i64TotalBytes);
...
void disk_get_geometry_win32(CHSgeometry_t *geom, HANDLE handle, const char *device, const int verbose)
{
    ...
    }
    {
                            ...
                            NULL))
        {
            ...
            {
                if (verbose > 1)
                    ;// log_verbose("IOCTL_DISK_GET_DRIVE_GEOMETRY %s Ok (%lu, %u, %u)\n", device, geom->cylinders,
                    //             geom->heads_per_cylinder, geom->sectors_per_head);
                return;
            }
        }
    }
...
std::optional<disk_t> file_test_availability_win32(const char *device, const int verbose, int testdisk_mode)
{
        ...
        return std::nullopt;
    {
        ...
        disk_car.pwrite =
            ((data->mode & FILE_WRITE_DATA) == FILE_WRITE_DATA ? file_win32_pwrite : file_win32_nopwrite);
