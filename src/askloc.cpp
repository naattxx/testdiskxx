...
void get_dos_drive_list(struct td_list_head *list)
{
    ...
    for (i = 'a'; i <= 'z'; i++)
    {
        ...
        new_drive = (file_info_t *)new unsigned char[sizeof(*new_drive)];
        new_drive->name = new char[4];
...
void ask_location(char *dst_directory, const unsigned int dst_size, const char *msg, const char *src_dir)
{
    ...
    do
    {
        ...
        }
        {
            ...
            file_info = (file_info_t *)new unsigned char[sizeof(*file_info)];
            while (1)
            {
                ...
                if (dir_entrie->d_name[0] == '.' && !(dir_entrie->d_name[1] == '\0') &&
                ...
                if (strlen(dst_directory) + 1 + strlen(dir_entrie->d_name) + 1 <= sizeof(current_file)
                    ...
                    && (memcmp(dst_directory, "/cygdrive", 9) != 0 ||
                        (dst_directory[9] != '\0' && dst_directory[10] != '\0') || dir_entrie->d_name[0] != '.')
#endif
                )
                {
...
#if defined(DJGPP) || defined(__OS2__)
                    if (current_file[0] != '\0' && current_file[1] != '\0' && current_file[2] != '\0' &&
                        current_file[3] != '\0')
#else
                    if (current_file[1] != '\0')
#endif
...
#endif
                    {
                        ...
                        /* Fix Drive list */
                        if (memcmp(dst_directory, "/cygdrive", 9) == 0 &&
                            (dst_directory[10] == '\0' || dst_directory[11] == '\0'))
                        {
                            file_info->st_mode = LINUX_S_IFDIR | LINUX_S_IRWXUGO;
            ...
            do
            {
                ...
                else
                {
                        ...
                        (dst_directory[9] != '\0' && dst_directory[10] != '\0'))
                    {
                        ...
                        if (cygwin_conv_path(CCP_POSIX_TO_WIN_A | CCP_ABSOLUTE, dst_directory, beautifull_dst_directory,
                                             sizeof(beautifull_dst_directory)) == 0)
                ...
                do
                {
...
#endif
                    {
                            ...
                            (LINUX_S_ISDIR(file_info->st_mode) || LINUX_S_ISLNK(file_info->st_mode)))
                        {
                            ...
                            {
#if defined(DJGPP) || defined(__OS2__)
                                if (dst_directory[0] != '\0' && dst_directory[1] != '\0' && dst_directory[2] != '\0' &&
                                    dst_directory[3] != '\0')
#else
...
static void dir_aff_entry(WINDOW *window, file_info_t *file_info)
{
    ...
    wprintw(window, "%s %5u %5u ", str, (unsigned int)file_info->st_uid, (unsigned int)file_info->st_gid);
