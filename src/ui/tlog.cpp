...
auto ask_testdisk_log_creation(App &app) -> TD_LOG
{
    ...
    auto aboutBuild = std::make_shared<Label>(
        StyledText("TestDisk++ ").bold(VERSION).add(", Data Recovery Utility, ").add(TESTDISKDATE));
    ...
    auto aboutTestdisk =
        std::make_shared<Paragraph>("TestDisk is free data recovery software designed to help recover lost "
    ...
    auto aboutLog =
        std::make_shared<Paragraph>(StyledText("Information gathered during TestDisk use can be recorded for "
