...
#include "src/next.hpp"
#include "src/part/ext2.hpp"
#include "src/part/fat32.hpp"
#include "src/part/md.hpp"
#include "src/part/ntfs.hpp"
#include "src/part/ntfs_struct.hpp"
...
/* Optimization */
static inline void offset2CHS_inline(const disk_t &disk_car,
                                     const uint64_t offset, CHS_t *CHS)
{
...
static inline auto CHS2offset_inline(const disk_t &disk_car, const CHS_t *CHS)
    -> uint64_t
{
  return (((uint64_t)CHS->cylinder * disk_car.geom.heads_per_cylinder +
           CHS->head) *
...
static auto get_location_boundary(const disk_t &disk) -> unsigned int
{
  if (disk.arch == &arch_mac)
    return 4096;
  if (disk.arch == &arch_sun)
    return disk.geom.heads_per_cylinder * disk.geom.sectors_per_head *
           disk.sector_size;
  return disk.sector_size;
}

static auto align_structure_aux(const uint64_t offset, const disk_t &disk)
    -> unsigned int
{
  unsigned int tmp;
  if (offset % (1024 * 1024) == 0)
    return 1024 * 1024;
  tmp = disk.geom.heads_per_cylinder * disk.geom.sectors_per_head *
        disk.sector_size;
  if (offset % tmp == 0 ||
      offset % tmp == (uint64_t)disk.geom.sectors_per_head * disk.sector_size)
    return tmp;
...
static void align_structure_i386(list_part_t &list_part, const disk_t &disk,
                                 const unsigned int align)
{
  for (auto part = list_part.begin(); part != list_part.end(); part++)
  {
...
static void align_structure(list_part_t &list_part, const disk_t &disk,
                            const unsigned int align)
{
...
void only_one_bootable(list_part_t &list_part,
                       list_part_t::const_iterator &part_boot)
{
  if (part_boot->status == STATUS_PRIM_BOOT)
    for (auto part = list_part.begin(); part != list_part.end(); part++)
    {
      if ((part != part_boot) && (part->status == STATUS_PRIM_BOOT))
        part->status = STATUS_PRIM;
    }
}
...
static int interface_part_bad_ncurses(disk_t *disk_car, list_part_t &list_part)
{
  ...
  if (disk_car.disk_size <= ((uint64_t)1 << (28 - 1)) &&
      disk_size >= ((uint64_t)1 << (28 - 1)))
  {
    wmove(stdscr, 8, 0);
    wprintw(
        stdscr,
  ...
  do
  {
    ...
    int car;
    list_part_t & parts;
    for (i = 0, parts = list_part; (parts != NULL) && (i < offset);
         parts = parts->next, i++)
      ;
    for (i = offset; (parts != NULL) && ((i - offset) < INTER_BAD_PART);
         i++, parts = parts->next)
    {
...
static auto interface_part_bad_log(disk_t &disk_car, list_part_t &list_part)
    -> int
{
  uint64_t disk_size = disk_car.disk_size;
  if (list_part.empty())
    return 1;
  {
    for (const auto &part : list_part)
    {
      disk_size = std::max(disk_size, part.part_offset + part.part_size - 1);
    }
  }
  log_warning("{}", disk_car.description(disk_car));
  log_warning(
      "Check the hard disk size: HD jumper settings, BIOS detection..."
  );
#if defined(__CYGWIN__) || defined(__MINGW32__)
...
static void warning_geometry_ncurses(
    disk_t *disk_car, const unsigned int recommanded_heads_per_cylinder
)
{
  aff_copy(stdscr);
  wmove(stdscr, 4, 0);
  ...
  wmove(stdscr, 7, 0);
  wprintw(stdscr, "but the correct value may be %u.",
          recommanded_heads_per_cylinder);
  wmove(stdscr, 8, 0);
...
static void hint_insert(uint64_t *tab, const uint64_t offset,
                        unsigned int *tab_nbr)
{
...
static void search_add_hints(const disk_t &disk, uint64_t *try_offset,
                             unsigned int *try_offset_nbr)
{
  if (disk.arch == &arch_i386)
  {
    ...
    hint_insert(try_offset,
                (uint64_t)16 * disk.geom.sectors_per_head * disk.sector_size,
                try_offset_nbr);
    hint_insert(try_offset,
                (uint64_t)17 * disk.geom.sectors_per_head * disk.sector_size,
                try_offset_nbr);
    /* 1/[01]/1 CHS x 240 63 */
    ...
    hint_insert(try_offset,
                (uint64_t)240 * disk.geom.sectors_per_head * disk.sector_size,
                try_offset_nbr);
    hint_insert(try_offset,
                (uint64_t)241 * disk.geom.sectors_per_head * disk.sector_size,
                try_offset_nbr);
    /* 1/[01]/1 CHS x 255 63 */
    ...
    hint_insert(try_offset,
                (uint64_t)255 * disk.geom.sectors_per_head * disk.sector_size,
                try_offset_nbr);
    hint_insert(try_offset,
                (uint64_t)256 * disk.geom.sectors_per_head * disk.sector_size,
                try_offset_nbr);
    ...
    if (disk.geom.cylinders > 1)
    {
      ...
      {
        start.cylinder--;
        hint_insert(try_offset, CHS2offset_inline(disk, &start),
                    try_offset_nbr);
      }
    }
    hint_insert(try_offset,
                (disk.disk_size - disk.sector_size) / (2048 * 512) *
                        (2048 * 512) -
                    disk.sector_size,
                try_offset_nbr);
  }
...
/*
   ...
   - Geometry: don't care
*/
static auto get_min_location(const disk_t &disk) -> uint64_t
{
  if (disk.arch == &arch_gpt)
    return 2 * disk.sector_size + 16384;
  if (disk.arch == &arch_i386 || disk.arch == &arch_humax)
    return disk.sector_size;
  if (disk.arch == &arch_mac)
    return 4096;
  if (disk.arch == &arch_sun)
    return (uint64_t)disk.geom.heads_per_cylinder * disk.geom.sectors_per_head *
           disk.sector_size;
...
static void search_NTFS_from_backup(disk_t &disk_car, list_part_t &list_part,
                                    const int verbose, const int dump_ind,
                                    const uint64_t min_location,
                                    const uint64_t search_location_max)
{
  auto *buffer_disk = new unsigned char[16 * DEFAULT_SECTOR_SIZE];
  partition_t partition(disk_car.arch);
  for (const auto &part : list_part)
  {
    if (part.upart_type == UP_NTFS && part.sb_offset != 0)
    {
      unsigned int i;
      for (i = 32; i > 0; i--)
      {
        const uint64_t tmp = i * disk_car.sector_size;
        if (part.part_offset > tmp)
        {
          partition.reset(disk_car.arch);
          partition.part_size   = (uint64_t)0;
          partition.part_offset = part.part_offset - tmp;
          if (disk_car.pread(disk_car, buffer_disk, DEFAULT_SECTOR_SIZE,
                             partition.part_offset) == DEFAULT_SECTOR_SIZE &&
              recover_NTFS(disk_car,
                           (const struct ntfs_boot_sector *)buffer_disk,
                           partition, verbose, dump_ind, 0) == 0)
          {
...
static auto search_part(disk_t &disk_car, const list_part_t &list_part_org,
                        const int verbose, const bool dump_ind,
                        const int fast_mode) -> list_part_t
{
  auto *buffer_disk  = new unsigned char[16 * DEFAULT_SECTOR_SIZE];
  auto *buffer_disk0 = new unsigned char[16 * DEFAULT_SECTOR_SIZE];
  partition_t partition(disk_car.arch);
  ...
  assert(disk_car.sector_size > 0);
  {
    ...
    for (const auto &part : list_part_org)
    {
      hint_insert(try_offset, part.part_offset, &try_offset_nbr);
    }
  }
  ...
  screen_buffer_reset();
  log_info("\nsearch_part()");
  log_info("{}", disk_car.description(disk_car));
  ...
  while (ind_stop != INDSTOP_QUIT && search_location < search_location_max)
  {
    ...
    if (disk_car.geom.heads_per_cylinder > 1)
    {
      if (old_cylinder != start.cylinder)
      {
        ...
        wclrtoeol(stdscr);
        wprintw(stdscr, "Analyse cylinder %5lu/%lu: %02u%%", start.cylinder,
    ...
    else if ((start.cylinder & 0x7FFF) == 0)
    {
      ...
      wprintw(stdscr, "Analyse sector %11llu/%llu: %02u%%",
              ...
              (long long unsigned)((disk_car.disk_size - 1) /
                                   disk_car.sector_size),
              (unsigned int)(search_location * 100 / disk_car.disk_size));
      wrefresh(stdscr);
      ask = 1;
      ...
      do
      {
        ...
        if (test_nbr == 0)
        {
          ...
          { /* Search Linux software RAID */
            if (disk_car.pread(disk_car, buffer_disk, 8 * DEFAULT_SECTOR_SIZE,
                               search_location) == 8 * DEFAULT_SECTOR_SIZE)
            {
              if (recover_MD(disk_car,
                             (const struct mdp_superblock_t *)buffer_disk,
        ...
        if (res <= 0 && test_nbr == 1)
        {
          if ((disk_car.arch == &arch_i386 &&
              ...
              (disk_car.arch != &arch_i386 &&
               (search_location % location_boundary == (7 - 1) * 512)) ||
              (disk_car.arch == &arch_gpt &&
               (search_location % (2048 * 512) == (7 - 1) * 512)) ||
              (disk_car.arch == &arch_none && search_location == (7 - 1) * 512))
            res = search_FAT_backup(buffer_disk, disk_car, partition, verbose,
                                    dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 2)
        {
          if ((disk_car.arch == &arch_i386 &&
               ((start.sector == 13 && (start.head <= 2 || fast_mode > 1)) ||
                search_location % (2048 * 512) ==
                    (13 - 1) * disk_car.sector_size)) ||
              (disk_car.arch == &arch_gpt &&
               (search_location % (2048 * 512) == (13 - 1) * 512)) ||
              (disk_car.arch != &arch_i386 &&
               (search_location % location_boundary ==
                (13 - 1) * disk_car.sector_size)))
        ...
        if (res <= 0 && test_nbr == 3)
        {
          if ((disk_car.arch == &arch_i386 &&
               ((start.sector == disk_car.geom.sectors_per_head &&
                 (start.head == disk_car.geom.heads_per_cylinder - 1 ||
                  fast_mode > 1)) ||
              ...
              (disk_car.arch != &arch_i386 &&
               search_location % location_boundary ==
                   (location_boundary - 512) &&
               search_location > 0))
            res = search_NTFS_backup(buffer_disk, disk_car, partition, verbose,
                                     dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 4)
          ...
          if ((disk_car.arch == &arch_i386 &&
              ...
              (disk_car.arch != &arch_i386 &&
               search_location % location_boundary ==
                   (location_boundary - 512) &&
               search_location > 0))
            res = search_HFS_backup(buffer_disk, disk_car, partition, verbose,
                                    dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 5)
        {
          ...
          /* It must be in fast_mode>0 because it can hide otherwise other
           * partition type */
          /* Block size: 1024, 2048 or 4096 bytes (8192 bytes on Alpha systems)
           */
          /* From e2fsprogs-1.34/lib/ext2fs/initialize.c:
           * set_field(s_first_data_block, super->s_log_block_size ? 0 : 1); */
          /* Assumes that TestDisk is not running under Alpha and
           * s_blocks_per_group=8 * block size */
          for (s_log_block_size = 0; (s_log_block_size <= 2) && (res <= 0);
               s_log_block_size++)
          {
            /* sparse superblock feature: The groups chosen are 0, 1 and powers
             * of 3, 5 and 7. */
            /* Checking group 3 */
            const uint64_t hd_offset =
                3 * (EXT2_MIN_BLOCK_SIZE << s_log_block_size) * 8 *
                    (EXT2_MIN_BLOCK_SIZE << s_log_block_size) +
        ...
        if (res <= 0 && test_nbr == 6)
        {
          ...
          {
            if (disk_car.pread(disk_car, buffer_disk0, 16 * DEFAULT_SECTOR_SIZE,
                               partition.part_offset) ==
                16 * DEFAULT_SECTOR_SIZE)
              res = search_type_2(buffer_disk0, disk_car, partition, verbose,
                                  dump_ind);
            else
              res = -1;
            test_nbr++;
        ...
        {
          if (res == 0)
            res = search_type_1(buffer_disk0, disk_car, partition, verbose,
                                dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 8)
        {
          if (res == 0)
            res = search_type_0(buffer_disk0, disk_car, partition, verbose,
                                dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 9)
        {
          res = search_type_8(buffer_disk, disk_car, partition, verbose,
                              dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 10)
        {
          ...
          res = search_type_16(buffer_disk, disk_car, partition, verbose,
                               dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 11)
        {
          res = search_type_64(buffer_disk, disk_car, partition, verbose,
                               dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 12)
        {
          ...
          /* Try to catch disklabel before BSD FFS partition */
          res = search_type_128(buffer_disk, disk_car, partition, verbose,
                                dump_ind);
          test_nbr++;
        }
        if (res <= 0 && test_nbr == 13)
        {
          res = search_type_2048(buffer_disk, disk_car, partition, verbose,
                                 dump_ind);
          test_nbr++;
        }
        if (test_nbr >= 14)
        ...
        if (res < 0)
        {
          ...
          wprintw(stdscr, "Read error at %lu/%u/%u (lba=%lu)\n", start.cylinder,
                  start.head, start.sector,
                  (unsigned long)(partition.part_offset /
                                  disk_car.sector_size));
#endif
...
#endif
          if (disk_car.arch->is_part_known(partition) != 0 &&
              partition.part_size > 1 && partition.part_offset >= min_location)
          {
            const uint64_t pos_fin =
                partition.part_offset + partition.part_size - 1;
            if (partition.upart_type != UP_MD &&
                partition.upart_type != UP_MD1 && ind_stop == INDSTOP_CONTINUE)
            { /* Detect Linux md 0.9 software raid */
              ...
              { /* disk_factor=1, detect Raid 0/1 */
                ...
                for (help_factor = 0;
                     help_factor <= MD_MAX_CHUNK_SIZE / MD_RESERVED_BYTES + 3;
                     help_factor++)
                {
                  const uint64_t offset =
                      (uint64_t)MD_NEW_SIZE_SECTORS(
                          (partition.part_size / disk_factor +
                           help_factor * MD_RESERVED_BYTES - 1) /
                          MD_RESERVED_BYTES * MD_RESERVED_BYTES / 512
                      ) *
                      512;
                  hint_insert(try_offset_raid, partition.part_offset + offset,
                              &try_offset_raid_nbr);
                }
              }
            ...
            {
              {
                partition_t new_partition = partition;
                int _insert_error;
                insert_new_partition(list_part, new_partition, 0,
                                     &_insert_error);
              }
              {
                const uint64_t next_part_offset =
                    partition.part_offset + partition.part_size - 1 + 1;
                const uint64_t head_size =
                    (uint64_t)disk_car.geom.sectors_per_head *
                    disk_car.sector_size;
                hint_insert(try_offset, next_part_offset, &try_offset_nbr);
                hint_insert(try_offset, next_part_offset + head_size,
                            &try_offset_nbr);
                if (next_part_offset % head_size != 0)
                {
                  hint_insert(try_offset,
                              (next_part_offset + head_size - 1) / head_size *
                                  head_size,
                              &try_offset_nbr);
                  hint_insert(try_offset,
                              (next_part_offset + head_size - 1) / head_size *
              ...
              if ((fast_mode == 0) &&
                  (partition.part_offset + partition.part_size -
                       disk_car.sector_size >
                   search_location))
              {
                search_location = partition.part_offset + partition.part_size -
                                  disk_car.sector_size;
                test_nbr        = 0;
                sector_inc      = 1;
              }
            ...
            {
              {
                partition_t new_partition = partition;
                int _insert_error;
                insert_new_partition(list_part_bad, new_partition, 0,
                                     &_insert_error);
              }
              if (verbose > 0)
                log_warning(
                    "This partition ends after the disk limits. (start={}, "
                    "size={}, end={}, disk end={})",
                    partition.part_offset / disk_car.sector_size,
                    partition.part_size / disk_car.sector_size,
                    pos_fin / disk_car.sector_size,
                    disk_car.disk_size / disk_car.sector_size
                );
              else
    ...
    else if (ind_stop == INDSTOP_PLUS)
    {
      ind_stop = INDSTOP_CONTINUE;
      search_location +=
          search_location_max / 20 / (1024 * 1024) * (1024 * 1014);
    }
    ...
    { /* Optimized "search_location+=disk_car.sector_size;" */
      ...
      if (min == (uint64_t)-1u || min <= search_location)
  ...
  if (fast_mode > 0)
    search_NTFS_from_backup(disk_car, list_part, verbose, dump_ind,
                            min_location, search_location_max);
  if (ind_stop != INDSTOP_CONTINUE)
    log_info("Search for partition aborted\n");
  if (!list_part_bad.empty())
  {
...
static auto reduce_structure(const list_part_t &list_part_org) -> list_part_t
{
  list_part_t list_part;
  for (const auto &part : list_part_org)
  {
    if (part.status != STATUS_DELETED)
    {
      partition_t new_partition = part;
      int _insert_error;
      insert_new_partition(list_part, new_partition, 0, &_insert_error);
    }
  }
...
static auto add_ext_part_i386(const disk_t &disk, list_part_t &list_part,
                              const int max_ext, const int verbose)
    -> list_part_t &
{
  ...
  for (auto part = list_part.begin(); part != list_part.end();)
  {
    if (part->status == STATUS_EXT)
    {
      /* remove already existing extended partition */
      order = part->order;
      part  = list_part.erase(part);
    }
    else
    {
      if (part->status == STATUS_LOG)
      {
        if (!deb.has_value())
        {
          deb = part;
  ...
  if (nbr_entries == 4 || max_ext != 0)
  {
    ...
    if (deb != list_part.begin())
    {
      uint64_t tmp;
      part_extended_offset = (*deb)->part_offset - disk.sector_size;
      if ((*deb)->part_offset % (1024 * 1024) == 0)
        tmp = 1024 * 1024;
      else
        tmp = (uint64_t)disk.geom.sectors_per_head * disk.sector_size;
      part_extended_offset = std::min(tmp, part_extended_offset);
    }
    else
    {
      uint64_t tmp;
      part_extended_offset =
          std::prev(*deb)->part_offset + std::prev(*deb)->part_size;
      /* round up part_extended_offset */
      if (std::prev(*deb)->part_offset % (1024 * 1024) == 0)
      {
        tmp = (part_extended_offset + 1024 * 1024 - 1) / (1024 * 1024) *
              (1024 * 1024);
      }
      else
      {
      ...
      if (tmp < (*deb)->part_offset &&
          (deb == list_part.begin() ||
           tmp >= std::prev(*deb)->part_offset + std::prev(*deb)->part_size))
        part_extended_offset = tmp;
    }
    if (std::next(*fin) == list_part.end())
    {
      part_extended_end =
          (*fin)->part_offset + (*fin)->part_size - disk.sector_size;
      /* In some weird cases, a partition may end after the end of the disk */
      part_extended_end =
          std::max(part_extended_end, disk.disk_size - disk.sector_size);
    }
    else
      part_extended_end = std::next(*fin)->part_offset - disk.sector_size;
    /* round down part_extended_end */
    if (part_extended_offset % (1024 * 1024) == 0)
    {
      const uint64_t tmp =
          part_extended_end / (1024 * 1024) * (1024 * 1024) - disk.sector_size;
      if ((*fin)->part_offset + (*fin)->part_size - disk.sector_size <= tmp)
  ...
  else
  {
    ...
    if (part_extended_offset % (1024 * 1024) == 0)
    {
      tmp = (part_extended_end + 1024 * 1024 - 1) / (1024 * 1024) *
                (1024 * 1024) -
            disk.sector_size;
    }
    else
  ...
  partition_t new_partition(disk.arch);
  new_partition.order = order;
  new_partition.part_type_i386 =
      (offset2cylinder(disk, part_extended_end) > 1023 ? P_EXTENDX
                                                       : P_EXTENDED);
  new_partition.status      = STATUS_EXT;
  new_partition.part_offset = part_extended_offset;
  new_partition.part_size =
      part_extended_end - new_partition.part_offset + disk.sector_size;
  int _insert_error = 0;
  insert_new_partition(list_part, new_partition, 0, &_insert_error);
  return list_part;
}

static auto use_backup(disk_t &disk_car, list_part_t &list_part,
                       const int verbose, const int dump_ind,
                       const unsigned int expert, char **current_cmd) -> int
{
  ...
  for (auto &part : list_part)
  {
    if (part.sb_offset != 0)
    {
      switch (part.upart_type)
      {
      case UP_FAT32:
        fat32_boot_sector(disk_car, part, verbose, dump_ind, expert,
                          current_cmd);
        break;
      case UP_NTFS:
        ntfs_boot_sector(disk_car, part, verbose, expert, current_cmd);
        break;
      case UP_HFS:
      case UP_HFSP:
      case UP_HFSX:
        HFS_HFSP_boot_sector(disk_car, part, verbose, current_cmd);
        break;
      default:
...
static void warning_geometry(const list_part_t &list_part, disk_t &disk,
                             const int verbose, char **current_cmd)
{
  if (!list_part.empty() && (disk.arch == &arch_i386 || disk.arch == &arch_sun))
  { /* Correct disk geometry is necessary for successfull Intel and Sun
       partition recovery */
    const unsigned int heads_per_cylinder =
        get_geometry_from_list_part(disk, list_part, verbose);
    if (disk.geom.heads_per_cylinder != heads_per_cylinder)
    {
      log_warning(
          "Warning: the current number of heads per cylinder is %u but the "
          "correct value may be %u.\n",
...
/*@
  ...
  @*/
static auto ask_write_partition_table(const list_part_t &list_part_org,
                                      disk_t &disk_car, const int verbose,
                                      const int dump_ind,
                                      const int ask_part_order,
                                      ...
                                      int *fast_mode) -> int
{
  ...
  int no_confirm         = 0;
  list_part_t list_part  = reduce_structure(list_part_org);
  /* sort list_part */
  sort_partition_list(list_part);
  ...
  do
  {
    do_again = 0;
    res_interface_write =
        interface_write(disk_car, list_part, ((*fast_mode) < 1),
                        can_ask_minmax_ext, &no_confirm, current_cmd, menu);
    ...
    case 'W':
      ...
      else if (disk_car.arch->write_part != nullptr)
      {
        if(no_confirm!=0
#ifdef HAVE_NCURSES
	    ...
	    )
        {
          ...
          {
            display_message(ftxui::Component(), msg_PART_WR_ERR);
          }
          else
          {
            use_backup(disk_car, list_part, verbose, dump_ind, expert,
                       current_cmd);
            if (no_confirm == 0)
              display_message(
                  ftxui::Component(),
                        ...
                        const bool ask_part_order, const bool expert) -> int
{
  ...
  do
  {
    ...
    if (fast_mode == 0)
      menu = 4; /* Search! */
    list_part = search_part(disk_car, list_part_org, verbose, dump, fast_mode);
    ...
    if (verbose > 0)
    {
      ...
      if (!list_part.empty())
        log_info(
            "\nHint for advanced users: dmsetup may be used if you prefer to "
            "avoid rewriting the partition table for the moment:\n"
        );
      for (unsigned int i = 0; auto &partition : list_part)
      {
        log_info("echo \"0 {} linear {} {}\" | dmsetup create test{}\n",
                 (long long unsigned)(partition.part_size / 512),
                 disk_car.device,
                 (long long unsigned)(partition.part_offset / 512), i++);
      }
#endif
    }
    // log_flush();
    do
    {
      ask_structure(disk_car, list_part, verbose, current_cmd);
      if (disk_car.arch->test_structure(list_part) == 0)
      {
        res_interface_write =
            ask_write_partition_table(list_part, disk_car, verbose, dump,
                                      ask_part_order, expert, current_cmd,
                                      &menu, &fast_mode);
      }
      else
      {
        display_message(ftxui::Component(), "Invalid partition structure.");
